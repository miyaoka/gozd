...
生成物は git に commit する。`buf.gen.yaml` でプラグインバージョンを pin して再現性を確保。
...
- **request**（応答あり）: `renderer → native`。ptySpawn, fsReadFile, switchDir 等
- **push**（一方向）: `native → renderer`。ptyText, gozdHook, fsChange, gitStatusChange 等
...
- `HookMessage`: Claude Code hooks からの状態通知。renderer に gozdHook を push
...
PTY 起動時に `ZDOTDIR` を gozd の `apps/native/Resources/zsh/v1/` に差し替え、gozd の初期化ファイルがユーザーの初期化ファイルを透過的に `source` する。
