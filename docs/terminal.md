...
`registerOscHandler(7)` で受けて leaf ごとの cwd を store（`cwdByLeafId`）に保持し、相対パスは
cwd 基準で絶対パス化してから worktree 内なら worktreeRelative（filer reveal が成立）、
外なら absolute（プレビューのみ）に解決する。

- cwd 未取得（OSC 7 を送らないシェル / chpwd 前）は worktree root 基準に fallback する
- cwd を component ローカルでなく store に置くのは title と同じ理由: OSC 7 は chpwd 時にしか
  流れず、ring buffer から evict された後の再マウントでは replay から復元できない
