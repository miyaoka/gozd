...
すべて `apps/electron/src/git/`（gitOps / gitBranch / gitLog / gitDiff / gitTree / gitBlame / worktreeOps / github）で実装され、`apps/renderer/src/features/*/rpc.ts` から呼ばれる。
...
| `rpcGitShowFile`        | HEAD 時点のファイル内容                | preview の diff 表示                                                                                                                                                                                                                                                                        |
...
| `rpcGitFetchRemotes`    | `git fetch --all` (全 remote)          | `useRemoteFetchSync` の背景 fetch (起動時/登録時は全 repo 初回 1 回、以降は active repo を focus 復帰 / 180s interval)。全 repo fan-out は `useRemoteFetchStore` の同時実行キューで cap され、超過ぶんは queue する (無制限並列は同一ホストへの TLS 接続バーストで connect hang を招くため) |
...
| RPC               | 用途                           | 呼ばれる場面                      |
| ----------------- | ------------------------------ | --------------------------------- |
| `rpcGitPrList`    | PR 一覧 (GraphQL)              | PR picker 起動時                  |
