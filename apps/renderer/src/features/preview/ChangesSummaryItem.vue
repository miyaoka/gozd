...
const BADGE_CLASSES: Record<GitChangeKind, string> = {
  ...
  untracked: "text-success bg-success/10",
...
<template>
  <div ref="rootRef" class="border-b border-border last:border-b-0">
    ...
    >
      ...
      <span v-if="props.change.type === 'R'" class="truncate text-xs text-foreground-subtle">
        ← {{ props.change.oldFilePath }}
      </span>
      <span
    ...
    <!-- 中身 -->
    <div v-if="!collapsed">
      <div v-if="loading" class="px-3 py-2 text-xs text-foreground-subtle">Loading...</div>
      <div v-else-if="error" class="px-3 py-2 text-xs text-destructive">{{ error }}</div>
      <div
        v-else-if="isBinary || isOriginalBinary"
      ...
      />
      <div v-else class="px-3 py-2 text-xs text-foreground-subtle">No diff</div>
    </div>
  </div>
</template>
