...
ターミナル全体のスクロールバックは保存しない代わりに、Claude Code セッションの sessionId だけを worktree 単位で永続化する（`~/.config/gozd/projects/<projectKey>/claude-sessions.json`）。`SessionStart` hook は CLI 経由で `session_id` / `transcript_path` を渡し、`upsert` で保存する。
...
`claude-sessions.json` からエントリが消える経路は 4 つ。削除の決定点は **ユーザーの明示的操作 / Claude の明示的終了** のみに置き、PTY exit そのものは削除契機にしない。アプリ終了時は renderer ごと死んで `unregisterPane` が走らないため、永続化はそのまま残って次回 resume できる。
...
| 起動時 `reconcileAll()`                                                          | アプリ起動時 1 回                              | `~/.config/gozd/projects/*/claude-sessions.json` を走査し、transcript ファイルが消滅したエントリを落とす（stderr に件数ログ）                         |
