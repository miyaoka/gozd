...
一覧が空のとき、**取得結果が空だったのか絞り込みで消えたのかを書き分ける**。同じ「何も無い」
でも、待てばよいのか語を変えればよいのかが変わる。
...
受理が走っている間、その行は選べず、進行中であることが行の上に出る。この表示は picker を
閉じて開き直しても残る。止まるのは同じ行だけで、別の行は並行して受理できる。

**ここで防いでいるのはブランチ名の衝突ではなく、同じ issue に worktree が 2 つできること**。
issue から起こすブランチ名は起動ごとに一意なので、二重の受理は失敗せずに重複を作る。
