...
`<doc>` はそのコンポーネントの **目的・責務・非自明な設計判断（なぜそうしているか）** を書く。実装そのもの
は書かない。判定基準は「**実装を変えたら追従が必要になる記述は書かない**」。構造は実装を見れば分かる。
...
書かない（実装変更のたびに追従が必要になり腐る）:

- ファイル / コンポーネント / サブフィーチャーの一覧、ディレクトリ構成、パス
- 「barrel が何を公開し何を隠すか」「どの子部品を持つか」などの構造説明
- 参照元・利用側の列挙（「〇〇 が使う」）
- Props / emit など実装から読み取れる情報
- コードの処理手順の逐条説明（「A して B して C する」）
