...
`TaskRow` の done / asking 吹き出しは行の上に被さる absolute overlay で、直前要素を覆う前提の
設計。WtCard ではカードヘッダがその受け皿になる。ここではグループラベルの行高とスクロール領域の
余白で同じ受け皿を用意する。無いと先頭グループの先頭行の吹き出しがスクロール上端で恒久的に
切れる（スクロールしても到達できない領域になる）。
