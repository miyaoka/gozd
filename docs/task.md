...
配布し、gozd が起動した Claude にだけ渡る — 撃てない場所では見えない。UI の一覧経由と同じものが出来上がる —
worktree、Task、自動起動、プロンプトへの事前挿入。違いは **一覧を介さず 1 件ずつ指定する**ことと、
**タイトルを呼び出し側が渡す**ことだけ。

- **タイトルは必須**。サイドバーで見分けの付かない Task を量産させない
- **GitHub 参照を指定したとき、その参照の Task が repo に既にあれば作らずに失敗を返す**。UI は
  既存 worktree への切り替えに倒すが、CLI には切り替える画面が無いため、既存の置き場所を添えて
  失敗として返す。一覧を読み直したエージェントが同じ番号を再投入しても増殖しない
- **事前挿入した指示は送信されない**。実際に走り出すのは人がその worktree を見て送った時点で、
  作成の成功はエージェントが動き出したことを意味しない
