...
- `hidden` は **サイドバー表示専用フラグ**。terminal close / SessionEnd で `ghRef` 持ち task を削除する代わりに `hidden=true` を立てる。`ghRef` 無し task は従来通り削除する。同 `worktreeDir` + 同 `ghRef` で `TaskStore.add` が来たとき (PR picker からの再選択は `pr.headRef` ベースで同 `worktreeDir` に hit する。issue picker は常に新規 worktree を作るためここに hit しない) や、`attachSession` で当該 task に attach したとき (resume クリック等) に `hidden=false` に戻り表示が復活する。identity (`ghRef`) と表示状態を分離することで「gh 系 / 直接起動系の terminal close 時挙動」を統一する
...
- 同 sessionId が既に attach 済み → 同一セッションの継続 (`claude --resume <sid>` 復帰) が確定している経路。当該 task が `hidden=true` なら `hidden=false` に倒してサイドバー表示を蘇生する (sessionID 一致は外から再現できない identity なので、別経路の素 claude が偶発的に取り憑くシナリオには該当しない)。それ以外は no-op (重複 hook / 復元レース)
- 同 `worktreeDir` で `sessionId == ""` かつ `hidden == false` の task のうち `createdAt` が最新のもの → attach
- 該当無し → 新規 task を UUID id で作成し sessionId を入れる (Claude 直接起動経路)

**候補ピックアップ (2 つ目の優先順位) で hidden な task を外す**のは、terminal close 済みの ghRef task に、別経路で起動した素 claude (新 sid) が偶発的に取り憑くのを構造的に防ぐため。sid 不一致での蘇生経路は「PR picker での再選択 → `TaskStore.add` の upsert」のみ (issue picker は branch を timestamp ベースにしているため常に新規 worktree を作り、sid 不一致蘇生経路を持たない)。sid 一致の蘇生は本ロジックの 1 つ目の優先順位が担う。
...
resume 失敗検出経路 (`claude --resume <sid>` が transcript 不在等で error 終了) で呼ばれる。`markHiddenIfGhRef` で caller の意図を切り替える。

- `ghRef` なしなら task ごと削除する (`markHiddenIfGhRef` 値は無関係)
- `ghRef` ありなら sessionId を空に書き換える。続いて:
  - `markHiddenIfGhRef=true` (terminal close 由来 / `removeByPty`): `hidden=true` を立ててサイドバー表示も消す。pane が閉じているので直後の `attachSession` は走らない。再表示は picker での再選択 (`add` の upsert) を待つ
  - `markHiddenIfGhRef=false` (session-start fallback 由来 / `applyClaudeSessionHook`): `hidden` は据え置く。直後の `attachSession(新 sid)` が `hidden=false` な ghRef task を拾って自動転移するため、ユーザー視点で連続性を保てる
