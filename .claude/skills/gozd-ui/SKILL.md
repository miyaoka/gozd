...
intent bg / border の alpha 値は **上記用法表の各行に書かれた値からのみ** 選ぶ
(bg: `/10` / `/15` / `/30` / `/40` / 無し、border: `/60` / 無し)。表外の値 (bg `/20` / `/25` /
`/50`、border `/40` / `/80` 等) は SSOT 違反として禁止。text 側の強度減衰 (`text-success/70`
等) は連続値で OK (情報の主役は色相 + 強度減衰)。raw 色との組み合わせ (`bg-red-500/20` 等)
は禁止 (intent token を使う)。
