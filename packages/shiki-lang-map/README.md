...
└── src/
    ├── index.ts                       # バレル (公開 API)
    ├── generateExtensionLangMap.ts    # codegen (prepare スクリプト)
    └── grammarLoad.test.ts            # 生成値の runtime smoke test
```
...
`pnpm install` 時に `prepare` で `bun src/generateExtensionLangMap.ts` が走り、`dist/extensionLangMap.generated.ts` を再生成する。
以下を実行する:
...
`bun src/generateExtensionLangMap.ts` 実行時に以下を stdout に出力する:
...
`package.json` で `shiki` を 2 箇所で参照している:

- `peerDependencies.shiki: ^4.0.0` — consumer に「shiki の major 4 系を持ち込むこと」を契約として表明する。生成物 `dist/extensionLangMap.generated.ts` の `import type { BundledLanguage } from "shiki"` を consumer が解決するのに必要
- `devDependencies.shiki: 4.0.2` (pinned) — generator script (`src/generateExtensionLangMap.ts`) が build 時に使う `bundledLanguagesInfo` / `BundledLanguage` の参照元。**生成テーブルの決定性を確保するため pinned**。caret にすると workspace の他 package が新しい shiki を持ち込んだ瞬間に生成内容が変わり、git 差分が install タイミングに依存してしまう

両者のバージョン range が乖離した場合 (例: consumer が shiki 5 に上げたが本 package を bump し忘れた)、`BundledLanguage` 型 / `bundledLanguagesInfo.id` 集合のズレが typecheck で検出される。consumer 側の shiki major bump 時は本 package の `peerDependencies` + `devDependencies` も同 major に合わせて bump する。
