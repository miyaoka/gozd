...
| PreviewPane summary `Close` ボタン             | `closeSummary`         | -                         |
...
- ChangesPane の `View all` 再押下と summary 内 `Close` ボタンはどちらも `usePreviewStore.closeSummary` に集約され、summary 解除と preview popover close を同時に実行する
