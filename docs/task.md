...
Shift+Enter / Shift+Click は picker を閉じずに accept し、複数の PR / issue からの連続作成に使う。作成完了時に picker item へ task が書き戻され、行が作成済み表示に変わる。別 item の accept は並行に走り、ブロックするのは branch が競合するケースだけ:

- 同一 item の accept 実行中の再選択 (行のチェックマーク位置にスピナー表示)
- issue picker で同一秒内の別 issue 選択 (branch 名が秒精度 timestamp のため同名衝突。in-flight branch 名で検知して notify)
