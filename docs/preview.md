...
- 画像 / SVG: 専用経路を持たず、テキストと同じ read RPC で取得した content から表示する。
  `FileReadResult.content` は「テキストは string、バイナリは生 bytes（`WireBytes`）」の union で、
  renderer が Blob → ObjectURL に変換して `<img>` に渡す（ImagePreview）。表示はデータ取得層の
  意味論（live の再取得 / pinned snapshot の固定）をそのまま反映し、URL 再読込やキャッシュバストは
  存在しない。Original タブも各取得経路（HEAD / 指定コミット / PR base）が実際に参照した rev の
  bytes が映る
- worktree 外の絶対パスは git 操作（`gitShowFile`）を呼ばず、`fsReadFileAbsolute` 単独で読み切る（画像 / SVG も同じ）
...
- バイナリ判定: NUL バイト（`0x00`）を含む or UTF-8 decode 失敗（main 側の read 実装が判定し、content の型で表現する。フラグは持たない）
