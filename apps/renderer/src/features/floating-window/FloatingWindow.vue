...
ウィンドウ操作ボタンはヘッダ右上の 1 グループに集約する: 中身固有のアクションは
`actions` slot、共通の promote / close はシェルが持ち、同一行に横並びになる。コードの所有権
(シェル / 中身) の境界を画面配置に漏らさないための契約で、header slot 内の任意の行に
ボタンを置くとウィンドウ操作が画面上に散る。actions slot に置くボタンは
...
// 外部 close 要求 (cmd+w 等) を close ボタンと同じ emit に合流させる。close の可否判断
// (未保存確認等) は consumer の @close ハンドラが担う (useFloatingWindows の doc 参照)。
watch(
