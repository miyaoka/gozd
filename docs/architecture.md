...
> [!NOTE]
> 60 秒間隔の `rpcGitRefsDigest` 整合性チェッカおよび PR 一覧の 60 秒ポーリングは廃止した。全 worktree watch + per-dir filter で SSOT 経路の到達率は実用的に十分であり、低頻度 pull は「予防的逃げ道」として GitHub rate limit の累積発火源になっていた。push 不達が観測されたら原因を直接修正する。
...
| -------------------- | --------------- | ----------- | -------------------------------------------------- |
| `SessionStart`       | `session-start` | CLI 経由    | `ptyId`, `session_id`, `transcript_path`, `source` |
| `SessionEnd`         | `session-end`   | CLI 経由    | `ptyId`, `session_id`, `transcript_path`           |
