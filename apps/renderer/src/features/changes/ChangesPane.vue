...
このパネルは Navigator の中にあり、その最小幅では要素を全部常時出せない。ヘッダを container にして、
狭いほど多く落とす段を持たせる。落とす順は件数 → セグメントのラベル → View all のラベル。閾値の値は
template が持つ。
...
**パネル名の隣にアイコンを置かない**。アイコンと名前は同じ「どのパネルか」を二重に運んでおり、最小幅
ではこの冗長が入らない。落とすのは冗長な側にする。同じ Navigator 列の Files ヘッダはアイコンを
持つが、あちらが幅を要求するのは名前と省略可能な状態テキストだけで、名前に加えて操作を 2 群持つのは
このヘッダだけ。
