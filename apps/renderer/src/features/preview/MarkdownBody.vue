...
- ` ```mermaid ` コードブロックは mermaid ライブラリで SVG にレンダリングする。mermaid は
  重量級なため dynamic import で読み込む。vite は `codeSplitting: false` のため別 chunk には
  分割されず単一バンドルに同梱されるが、mermaid のトップレベル評価と `initialize()` は
  mermaid ブロックが現れるまで実行されない (起動時の評価コストを避ける)
...
/**
 * ```mermaid コードブロックを後処理用のプレースホルダ div に変換する marked extension。
 * 実際の SVG 化は DOM 反映後に renderMermaidBlocks() が dynamic import した mermaid で行う。
 * source は encodeURIComponent して data 属性に載せる (改行 / 引用符で属性が壊れないように)。
 */
const mermaidExtension: MarkedExtension = {
  renderer: {
    code({ text, lang }) {
      if (lang !== "mermaid") return false;
      return `<div class="_mermaid-block" data-mermaid-source="${encodeURIComponent(text)}"></div>`;
    },
  },
};
...
/** プレースホルダ div を実 SVG に差し替える。parse error は block 内にテキストで表示する。 */
async function renderMermaidBlocks() {
  const root = bodyEl.value;
  if (root === undefined) return;

  const blocks = root.querySelectorAll<HTMLElement>("._mermaid-block[data-mermaid-source]");
  ...
  for (const block of blocks) {
    ...
    // 描画済みブロックの二重レンダリングを防ぐため、処理後は data 属性を外す。
    delete block.dataset.mermaidSource;

    const id = `mermaid-${mermaidIdCounter++}`;
    const result = await tryCatch(mermaid.render(id, source));
    if (result.ok) {
      block.innerHTML = result.value.svg;
      continue;
    }
    block.textContent = result.error instanceof Error ? result.error.message : String(result.error);
    block.classList.add("_mermaid-error");
  }
}
...
/* mermaid 図は中央寄せし、生成 SVG が幅を超えないように収める。 */
...
/* parse error 時はコードブロックと同じ地色で error 色のテキストを表示する。 */
._markdown-body :deep(._mermaid-error) {
  display: block;
  padding: 1em;
  border-radius: 6px;
  background: var(--md-code-bg, var(--color-panel));
