<doc lang="md">
端末 1 つ分の描画実体。PTY との入出力を繋ぎ、端末の中でしか観測できない事実を外へ渡す。

## 端末出力は信用しない

画面に出ている文字列は、端末の中で動くプログラムが自由に書けるものであって gozd の管理下に
ない。**リンクの形をしていても、素のクリックでは開かない**。UI が描いたリンクとは前提が違い、
起動に要する操作も違う。

## 観測はここ、解釈は外

外へ渡すのは、端末が名乗ったタイトル、シェルが遷移した作業ディレクトリ、いま画面に見えている
本文。いずれもここでしか観測できない。**渡した先での解釈はここでは持たない** — タイトルを
どの表示に使うか、本文から何を読み取るかは、受け取る側がそれぞれ決める。

焦点も同じ分界で、どの端末に焦点を置くかは上位が決め、ここは要求を受けて当て、端末側で起きた
変化を報告する。
</doc>
