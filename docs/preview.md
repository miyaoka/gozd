...
- 外側クリックでは閉じない
- Cmd+W の close チェーンは**フォーカス**で分岐する: terminal にフォーカスがあれば `terminal.closePane`、それ以外は最前面のサーフェス（preview popover 等）を閉じる（各コマンドの keybinding の when 条件）。undocked preview window の Cmd+W / Cmd+S も**フォーカス**で宛先が決まる: フォーカスが in-app パネル内なら `floatingWindowFocused` で `floatingWindow.close` / `floatingWindow.save`、promote 後の OS ウィンドウ内なら `childWindowFocused` で `childWindow.close` / `childWindow.save`（keybinding 系は child の document にも同一 dispatcher を張る。[keybinding.md](keybinding.md) の解決フロー）に解決され、どちらも traffic light と同じガード経路 / window ローカル保存に合流する。どのパネルにもフォーカスが無い状態の Cmd+W は最前面のサーフェスを閉じる — popover が開いていれば `preview.close`、開いていなければ `floatingWindow.closeFront`（最前面パネル 1 枚の close 要求）。同一キーは優先順位ではなく実効条件で排他にする契約なので、この分岐は各コマンドの when に書き分けられている。この意味論を成立させるため、popover の open（表示中のファイル切替を含む）では MainLayout の watch がフォーカスを popover へ移す — `showPopover()` はフォーカスを移さないため、terminal リンク経由の open でフォーカスが terminal に残ると、開いた直後の Cmd+W が preview でなく terminal pane を閉じてしまう。close 時は open 時点のフォーカス元へ戻す（terminal リンクから開いて Cmd+W で閉じると terminal に入力が戻る）。popover 内部にフォーカスがあるとき（Monaco 編集中等）は奪わない
...
- Filer 経由で PR diff に含まれない (merge-base 起点で無変更の) ファイルを選択したケースは、`orderedFileChanges` に該当 change が無いため working tree の中身を `rpcFsReadFile` だけで Current に出して救済する。「変更種別 → 利用可能モード」対応表 (Original タブの節の上) の「変更なし」行と対称。空表示に倒すと Filer で見えているファイルが PR モード中だけ読めなくなる非対称が生じるため。from (base) 側を撃たないので存在しないパスへの `git show` による bogus stderr も出ない
- ただし「change 無し」には PR diff 一覧の fetch 未確定な race も混ざる (enable トグル直後は source が prDiff に切り替わるが fetchedFiles が空で、diff 内の tracked ファイルも一時的に消える)。これを Current 救済に倒すと、変更済みファイルを開いたまま PR モードをトグルした際にデフォルトタブが Diff から Current に落ちる。`useChangesStore.loading` で判別し、fetch 中は救済せず一覧確定 (= `orderedFileChanges` の identity 変化) による再発火に委ねる
...
draft の生存は **preview が表示されている間だけ**。不変条件は「セッションが存在 ⇔ popover 表示中 && summary 外 && 編集可能 content 表示」で、両方向を別の場所が担う: 畳む側は `close()`（popover 閉 ⇒ セッション破棄。不可視の未保存 draft を残すと dirty 表示のないまま外部変更同期で無警告破棄される経路が生まれるため）と summary 進入、張る側は `usePreviewEdit` の同期 watch（可視状態 `isOpen` / `summary.enabled` もソースに含む — 再 open / summary 退出は content が変化しないため、可視状態自体を発火源にしないと張り直せない）。
