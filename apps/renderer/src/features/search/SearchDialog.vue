...
## 打ち切りは 2 段あり、どちらも黙って行わない

一致の収集と、その描画のそれぞれに上限がある。**収集側**は検索そのものが返す件数の上限で、
**描画側**は結果を仮想化しないため一致が数万行に達すると描画が破綻することによる。

**どちらの打ち切りも件数表示に現れる**。件数だけを見て「全部出ている」と読まれると、無いものを
無いと判断してしまう。ここは 2 つを区別して出す — どちらで切れたかによって、絞り込むべきか
別の語で引き直すべきかが変わる。
