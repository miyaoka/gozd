...
| RPC                     | 用途                                   | 呼ばれる場面                                                                                                           |
| ----------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `rpcGitStatus`          | working tree の status snapshot        | 初回 load、`gitStatusChange` push 後の再 fetch                                                                         |
...
| `rpcGitFetchRemotes`    | `git fetch --all` (全 remote)          | `useRemoteFetchSync` の背景 fetch (起動時/登録時は全 repo 初回 1 回、以降は active repo を focus 復帰 / 180s interval) |
...
| `rpcGitResetMixed`      | `git reset --mixed <hash>` 相当        | GitGraphPane の commit 行右クリックメニュー                                                                            |
...
| `worktreeChange`   | `worktrees/*` 配下、main worktree (root) 自身の branch 切替 (files backend は `.git/HEAD`、reftable backend は共有 `reftable/*`) | useSidebarData (source dir の repo)                                                                                                                |
...
発火源の ref パスは git の ref backend 2 種を両方トリガーに含む。`classify` は真値を porcelain (`git worktree list --porcelain` / `git status --porcelain=v2 --branch`) で読み直すため、ここでの分類は「どの porcelain を再取得すべきか」の signal でしかなく、backend の物理 layout に白名簿を焼き込むと layout 変化で無分類 silent drop が起きる。

- **files backend** (loose `refs/` + `packed-refs`): branch 切替は per-wt の `HEAD` (symbolic ref 先) だけが変わり `refs/heads/*` は動かない。main worktree (root) の `HEAD` は `.git/HEAD`、secondary の `HEAD` は `.git/worktrees/<name>/HEAD` に置かれる。後者は root watcher が `worktrees/*` 規則で `worktreeChange` として拾うが、root の `.git/HEAD` は per-wt 規則で `gitStatusChange` にしか乗らず worktree list が refetch されない非対称があった。そのため root (`perWorktreeGitDir == commonGitDir`) の `HEAD` 変化では `worktreeChange` も立てて branch label を更新する
- **reftable backend** (Git 2.51+、3.0 で default 化): ref はバイナリテーブル `reftable/` に格納され、`HEAD` スタブは `ref: refs/heads/.invalid` 固定で動かない。branch 切替・作成・削除・rename・fetch がすべて `reftable/` の書き換えに funnel され、local / remote / HEAD を種別判別できない。そのため `packed-refs` と同じく共有 `reftable/*` は `branchChange` + `gitStatusChange` + `remoteRefsChange` を全発火させる。加えて root (`perWorktreeGitDir == commonGitDir`) では `worktreeChange` も立てる。reftable では HEAD スタブが動かず、root の `git switch existing-branch` を branch label に反映する唯一の signal がこの共有テーブル変化のため、files backend の `HEAD` → `worktreeChange` 規則と対称化する。既存 branch 切替は `refs/heads` の OID/集合を動かさず `branchChange` が heads-digest gating で抑止される一方、`worktreeChange` は gating 対象外で primary が無条件 dispatch するため、これが branch label 更新の正規経路になる。per-wt の `reftable/*` (secondary の `.git/worktrees/<name>/reftable/`) はその worktree の checkout 先変化として `gitStatusChange` を立て、branch label の list 再取得は root watcher の `worktrees/*` 規則が担う
