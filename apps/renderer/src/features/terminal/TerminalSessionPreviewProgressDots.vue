...
(暴走・ループ) の signal であり、溢れを抑え込むとその発見が遅れる。点の列がスクロール面の
可視域を占め、会話バブルが初期表示から外れる (スクロールすれば見える) のはこの signal と
引き換えに受け入れている。スクロール面は `flex-col-reverse` で末尾 (= この吹き出し) に
アンカーされるため、押し出されるのは会話側になる。
