...
//         内部の変更は外側の git status 出力を変えない。submodule は内部の変更が外側の status に
//         gitlink の変更として現れるため対象外。ここで status を取り直すと、入れ子側の
...
export interface ClassifyInput {
  ...
  /** dir 配下に入れ子で置かれた、同じ repo の別 worktree root（realpath 解決済み）。
