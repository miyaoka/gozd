...
| feature 内部   | `features/*/README.md`      | コンポーネントを持たない feature の実装詳細（変換ルール、文法、モジュール間依存） | shared/command/README.md — パーサー文法、除外判定ロジック        |
...
`eslint-plugin-boundaries` がこのルールを強制する。違反すると lint エラーになる。
