...
- **PR 経由**: その PR の head ブランチを起点にする。**ブランチが PR と 1:1 なので、
  1 つの PR に持てる worktree は 1 つ**
...
**PR の一覧だけは既存へ切り替える。** PR 由来の worktree は head ブランチを指すため、
git が同じブランチの 2 重 checkout を拒否する。作れない以上、選択は既存への移動になる。
...
- **GitHub 参照を指定しても、その参照に既に worktree があるかは見ない**。同じ番号を何度
  ...
  おり、既存への移動を求めていない。起点ブランチが参照に依存しないため作成は常に成立する
