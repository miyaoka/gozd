...
ソケットのプロトコルは NDJSON で、1 行が 1 メッセージ。メッセージは種別を表すフィールドを
1 つだけ持つ。種別はパスを開く要求、Claude Code の状態通知、worktree の作成要求と、窓口に渡す
操作（repo とセッションの問い合わせ、セッションを開く要求、worktree の削除要求。
[concierge.md](concierge.md)）。

処理は逐次キューで直列化し、**同一 PTY のセッション系 hook が送信順に処理されること**を保証する。
decode 失敗は接続を切らずログに残す。

**応答は種別で決まる。** 状態を通知するだけの種別は送りっぱなしにし、実行者が結果を知らないと
次へ進めない種別だけ、同じ接続に 1 行返してから閉じる。パスを開く要求と状態通知以外の種別が
これにあたる。処理が失敗したときも失敗の応答を返し、実行者を応答待ちのまま残さない。

応答を返す側は、クライアントが送信直後に送る FIN で書き込み側まで閉じないようにする（half-open
を許可する）。閉じてしまうと、非同期処理を終えた応答が書けずに落ちる。

**応答は「main が実行できたか」だけを表す。** UI への反映は push に委ねるため、応答の成功は
画面に出たことを意味しない。逆に、renderer が居ない状態でも作成や削除そのものは成立する。

**ソケットに書ける主体は、gozd と同じ権限でエージェントを起動できる。** worktree の作成要求は
指示文を伴い、起動したエージェントはそれを受け取って動き出す。ソケットはユーザー専用の
一時ディレクトリに置かれるため到達できるのは同一ユーザーだけで、その主体は元より同等の
実行権限を持つ。この前提が崩れる配置（共有された一時ディレクトリ等）へ移すときは、
受け付ける種別を見直す。

同じ理由で、**種別ごとの要求元の制限は誤操作の防止であり、セキュリティ境界ではない**。要求元の
端末は要求に載る ID で決まり、送り手が自分で書ける。
...
| worktree サブコマンド | 稼働中のウィンドウ前提。未起動なら cold start させず接続失敗で終える   |
| repo / session        | worktree サブコマンドと同じ                                            |
...
- **repo 内に置かれた同じ repo の別 worktree（入れ子 worktree）の変更で、外側の git status を
  ...
  取り直すと、入れ子側での依存 install やビルドのたびに外側 repo の git status が走る。
  外側への変更通知（ファイラーの更新）は届ける
...
外部を開く経路は 1 本に集約する。リンククリックを受け取る層はすべてこれを通し、allowlist 外は
開かずに拒否して失敗を通知に倒す。

**外部リンクをブラウザ既定の挙動に委ねない**。防壁は新規ウィンドウ要求を URL を見ずに拒否するため、
委ねた先が存在しない。リンクを描く層が自分で外部送りを起こさないと、そのリンクは無音で死ぬ。
