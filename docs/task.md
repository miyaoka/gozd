...
- **この経路への並行投入でも増えない**。後から来たほうは先行の作成完了を待ってから判定する。
  その参照の worktree が既にあれば、置き場所を添えて失敗する。参照が違えば互いに待たない
- **一意性の保証は 1 つのインスタンスの、この経路の中に閉じる**。一覧からの作成と並行したとき、
  および複数インスタンスを同時に動かしたときは、同じ参照に 2 つ作られうる
