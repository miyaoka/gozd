...
ChangesPane ヘッダの `PR #<n>` toggle が ON のとき、summary を含む各 view は「`pr.baseRefOid` から working tree まで」(base..working) の diff に切り替わる。詳細:

- 一覧は `git diff <baseOid>` (`olderIsBase=true`、`includeWorkingTree=true`) + untracked を merge した形で `useChangesStore.fileChanges` に乗る
- per-file diff は `rpcGitShowCommitFile`(from = baseOid の blob) + `rpcFsReadFile`(to = working tree) を並列取得して描画する
- toggle 経路の SSOT は `usePrDiffToggleStore`。`gitGraphStore` の selection は触らず、`selectionVersion` の increment (= ユーザーが graph で commit を選んだ) で自動 OFF になる
- PR が見つからない / `baseRefOid` 未解決のときは toggle 自体が gate される (`canEnable === false`)
- base OID が local に reachable でないとき (未 fetch) は `rpcGitFetchRemotes` で 1 回自動 retry する
