...
`draggable` は in-app パネル用で、ヘッダ全体をドラッグハンドルにする cursor だけを切り替える
(pointerdown の listener は attribute fallthrough で親が張る)。ウィンドウ操作ボタン
...
<template>
  <header
    ...
    :class="draggable === true ? 'cursor-grab select-none active:cursor-grabbing' : ''"
  >
    <slot name="header" />
