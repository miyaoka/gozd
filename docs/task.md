...
- **同じ参照への並行投入でも増えない**。既存の判定と作成は参照ごとに逐次化し、後から来たほうが
  必ず先行の書き込みを見てから判定する。参照が違えば待ち合わせない。**永続レコードでは守れない**
  — worktree の作成が Task の書き込みより先なので、Task 書き込み時に衝突を見つけても worktree は
  既に出来ている。錠前は作成より前に取る必要があり、プロセス内に閉じる
