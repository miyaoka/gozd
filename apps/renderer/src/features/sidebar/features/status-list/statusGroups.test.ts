...
describe("groupByStatus", () => {
  ...
  test("端末が開いている行は repo ごとにまとめ、注意が要る行を持つ repo を上に置く", () => {
    const groups = groupByStatus([
      row("a-idle", { rootDir: "/a", status: { state: "idle", lastActivityAt: 1 }, stateSince: 9 }),
      row("b-idle", { rootDir: "/b", status: { state: "idle", lastActivityAt: 1 }, stateSince: 8 }),
      row("b-asking", {
        rootDir: "/b",
        status: { state: "asking", lastActivityAt: 1 },
        stateSince: 1,
      }),
      row("a-done", { rootDir: "/a", status: { state: "done", lastActivityAt: 1 }, stateSince: 2 }),
    ]);
