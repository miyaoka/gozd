...
- 行番号 button クリックで `lineNumberClick` イベントを emit（`usePreviewRevs` が BlamePopover に橋渡し）
...
- `usePreviewContent` (fsChange の `onBeforeRefetch` hook) / ChangesSummaryItem の `fsChange` callback は `fetchContent()` / `runFetch()` の **前** に `useBlamePopover().closeIfActive(dir, relPath)` を発火する。content 更新で CodePreview の Shiki / fallback button や DiffPreview の line-no button が DOM 置換されると anchor が detached になるため、再描画と同フレームで popover を閉じる
...
- ファイル内容を `srcdoc` で `<iframe>` に流し込み WebKit にネイティブ描画させる
- `sandbox=""`（全権限なし）を必須とする。`srcdoc` iframe は**デフォルトで親 origin を継承する**ため、sandbox を外すと iframe 内 JS が renderer と同 origin（`gozd-app://localhost` / dev origin）で動き、`gozd-rpc://` の CORS allowlist を通過して任意ファイル bytes を回収できてしまう
  - `sandbox=""` で origin を opaque 化すると `<script>` は実行されず（静的 HTML + CSS のみ描画）、仮に scripts を許可しても opaque origin は allowlist に含まれず構造的に RPC を叩けない
