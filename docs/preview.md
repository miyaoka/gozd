...
- Cmd+A の scope 制御は **focus が leaf 内に居るときの挙動** に限定される。focus がトグル / タブ / ヘッダボタン等の leaf 外要素に乗っているときの Cmd+A は WebKit が document scope に倒し、preview popover 外を含む document 全体の `user-select: text` 領域が選択される。これは「Cmd+A を JS で intercept しない」「`user-select: none` を scope 制御に使わない」(WebKit の selectAll は user-select を honor しない) 方針から構造的に残る帰結。leaf 外 focus 時に preview 内に閉じたい場合はユーザーが leaf を 1 回クリックして focus を移す前提
