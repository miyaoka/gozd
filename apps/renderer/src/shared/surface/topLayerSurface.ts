 ...
 * 落とすので、積み直しの対象にフォーカスがある状況は普通に起きる (前面化はクリックだけでなく
 * open でも起きるため、「最前面 = フォーカスがある」は成立しない。例: preview を編集中に
 * タイトルバーのボタンで別パネルを開くとフォーカスは preview に残る)。それでも退避が不要なのは、
 * 積み直しの発火源がそのサーフェスへの pointerdown であり、伝播完了後の既定動作が同じサーフェス
 * 内へフォーカスを戻すため。**この前提はサーフェスの root が focusable であることに依存する** —
 * クリック先が focusable でないとき、既定動作は最も近い focusable な祖先を選ぶので、root が
 * focusable でないとフォーカスが body へ抜ける。全サーフェスの root に `tabindex="-1"` が要る。
 *
 * ## pin (常時最前面)
 *
