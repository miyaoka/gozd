...
/** HEAD マーカー: lane 0 の左隣から dot を指す右向き三角形。apex が右にどれだけ伸びるか / base の縦幅 (px)。 */
const HEAD_MARKER_WIDTH = 8;
const HEAD_MARKER_HEIGHT = 12;

/**
 * graph 列の右側に確保するガター (px)。最右レーンの dot が隣の description 列に
 * 密着しないための余白。HEAD はどの dot かを指す右向き三角マーカーで示す (グラフ座標系に置く)。
 */
...
/**
 * HEAD マーカーの polygon points。グラフ座標系の x=0 に縦の base を立て、apex を右に尖らせた
 * 右向き三角形。HEAD は表示集合内で常に lane 0 に固定されるため、この位置は lane 0 の dot の
 * 左隣になり、どの dot が HEAD かを指す矢印として読める。
 */
