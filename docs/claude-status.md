...
人間に通知してよい「真の done」の条件は、**システム全体の静止（quiescence）**である:
...
既知の残存リスク（hook にメッセージ可視性がない以上、マーカーは近似であるため）:

- **通知 drop**: 再起動が来ないと working 表示が残る。`pendingWork` と同型の故障モードで、状態機械は `done` を経由しているため `clearDoneStates`（フォーカス時の既読消化）で救済される
- **複数 teammate の同時 idle**: マーカーは ptyId 単位の boolean のため、複数の idle 通知が別ターンに分かれて配送されると 2 通目以降で完了通知が再度鳴りうる（配送粒度は hook から観測できない）。失敗様態は「通知が余分に鳴る」で、状態固着はしない
- **hook 到達順の逆転**: `subagent-start` の socket 到達が同ターンの `Stop` より遅れると、終局判定が 1 ターン早まりうる。socket の逐次キューが submit 順を保証しており通常は起きない
