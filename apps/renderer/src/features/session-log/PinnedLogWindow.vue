...
- リサイズは 8 方位の不可視ハンドル + pointer capture (ネイティブ `resize: both` は仕様で
  ...
  ずれると、左/上辺リサイズで CSS だけが勝った瞬間にアンカー辺が滑る
...
function onResizePointerMove(event: PointerEvent) {
  ...
  const root = rootRef.value;
  if (root === null) return;
  const { dir, startX, startY, startRect, minWidth, maxWidth, minHeight, maxHeight } = resizeState;
  const dx = event.clientX - startX;
  const dy = event.clientY - startY;
  // 左/上辺は反対辺アンカー: サイズを先にクランプしてから位置を逆算する。位置を先に
  // 動かすと min/max に当たった瞬間にアンカー辺がずれて window が滑る。
  // x/y の基準は store の保存座標ではなく実測 rect (描画クランプで押し戻されていても
  // その場から動く。ヘッダドラッグのオフセット実測と同じ理由)。
  let x: number | undefined;
  let y: number | undefined;
  if (dir.includes("e")) {
    root.style.width = `${clamp(startRect.width + dx, minWidth, maxWidth)}px`;
  }
  if (dir.includes("w")) {
    const width = clamp(startRect.width - dx, minWidth, maxWidth);
    root.style.width = `${width}px`;
    x = startRect.right - width;
  }
  if (dir.includes("s")) {
    root.style.height = `${clamp(startRect.height + dy, minHeight, maxHeight)}px`;
  }
  if (dir.includes("n")) {
    // 上端はタイトルバー (アプリの drag 領域) 直下まで。ドラッグの y クランプと同じ下限
    const limit = Math.min(maxHeight, startRect.bottom - TITLEBAR_HEIGHT);
    const height = clamp(startRect.height - dy, minHeight, limit);
    root.style.height = `${height}px`;
    y = startRect.bottom - height;
  }
