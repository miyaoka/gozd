...
Markdown preview 内の内部リンククリックは back / forward 履歴に積まれる。ブラウザの「戻る / 進む」と同じモデルで、`useMarkdownHistoryStore` の双方向スタックが保持する。

- リンククリック時: 現在の `selection` を back スタックに push し、forward スタックを破棄する
- back ボタン: back から pop して forward に push、selection をその値に戻す
- forward ボタン: forward から pop して back に push、selection をその値に進める
- PreviewPane ヘッダの矢印ボタン (`icon-[lucide--arrow-left]` / `icon-[lucide--arrow-right]`) を常時表示し、履歴が無いときは disabled にする (履歴の有無で header の幅が揺れないように)
- キーバインド: `cmd+[` (back) / `cmd+]` (forward)。`when: "previewVisible && !inputFocused"` でゲートし、preview popover が開いていてかつ入力欄にフォーカスがないときだけ発火する
- コマンドパレットからは `Markdown Preview: Go Back` / `Markdown Preview: Go Forward` で実行可能 (precondition `previewVisible`)

履歴のスコープは「md preview 内部リンク経由の遷移」に限定する。filer クリック / terminal リンク / プログラム的な `selectFromTarget` 呼び出しなど、それ以外の経路で `worktreeStore.selection` が変化した瞬間に履歴は破棄される。実装は `worktreeStore.selection` を `flush: 'sync'` で watch し、自前の navigate / goBack / goForward 経路で立てた `isInternalNav` フラグが立っていなければ「外部遷移」とみなして両スタックをクリアする。dir 切替 (`worktreeStore` 側で selection が undefined に倒される) も同じ外部経路として履歴クリアに繋がる。
