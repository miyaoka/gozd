...
Shift+Enter / Shift+Click は picker を閉じずに accept し、複数の PR / issue からの連続作成に使う。作成完了時に picker item へ task が書き戻され、行が作成済み表示に変わる。別 item の accept は並行に走り、ブロックするのは同一 item の accept 実行中の再選択のみ (同じ branch の二重作成になるため。実行中の行はチェックマーク位置にスピナー表示)。同一秒内の並行作成による timestamp branch 名の衝突は、`generateTimestamp` (`@gozd/shared`) が per-process 一意 (同一秒は連番 suffix) を保証するため起きない。
