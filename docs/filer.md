...
> [!NOTE]
> 削除によって preview 表示中のファイルが消えた場合の自動クローズは filer ではなく preview 側の責務。PreviewPane が再 fetch で current / HEAD いずれにも無いと判定したときに閉じる（[preview.md](preview.md#開閉機能) 参照）。filer はツリー表示の更新のみを担う。
...
- NavigatorPane の Filer ヘッダーは compact な時刻表示を 1 つ持つ（snapshot mode では選択中コミットの日時、working tree mode では変更ファイルの最新 mtime）。snapshot mode 中はこれに加えて "Now" ボタンを表示する。"Now" は `gitGraphStore.select(UNCOMMITTED_HASH)` を呼び、GitGraphPane の Working Tree 行クリックと同じ経路で working tree モードに戻す。「今 snapshot を見ている」ことと「working tree へ戻る」手段を、GitGraphPane 側を操作せず filer 側単独で完結させる
