...
| ChangesPane `View all` (summary 解除)          | `close`                | -                         |
...
- ChangesPane の `View all` を再押下すると summary は解除され、preview popover も同時に閉じる (トグル契約)。`summary 解除 → 単一ファイル表示にフォールバック` は採用しない: View all 経由でしか summary を開いていない場合、選択中ファイルが無く placeholder だけが残るため、トグル off は popover ごと閉じる方が意図と一致する
- summary 内の Close ボタンも summary 解除 + popover close で同じセマンティクス (`View all` トグル off と等価)
