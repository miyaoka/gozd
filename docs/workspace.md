...
モーダルを inert として扱う既存の受容範囲（`SettingsModal` の doc）。

閉じる順序も同じ規則に従う。ESC と Cmd+W は同義で「最前面のサーフェスを閉じる」。開く順序と
閉じる順序が別の規則を持つと、見えている前面と操作対象が食い違う。

実装は `shared/surface` に集約する。開閉も前面化も close の宛先解決もここを通す契約で、迂回すると
「今どれが最前面か」の判定が壊れる。順序が z-index ではなく show 順で決まる理由と、積み直しが
見た目のコストを持たない理由は同モジュールの docstring が SSOT。
