...
- **PR 経由**: その PR の head ブランチを**そのまま使う**。git は同じブランチを 2 つの
  worktree に checkout できないため、**この経路からは同じ PR の 2 本目を作れない**。
  制約はブランチに掛かるもので、参照に掛かるものではない
- **issue 経由**: ブランチ名はタイムスタンプ形式（通常の新規作成と同じ）。**issue は head ref を
  持たないため worktree との 1:1 をブランチ名に埋め込まない** — 埋め込むと、Task を消した後に
  同じ issue から作り直したときブランチ名が衝突する。**同じ issue に worktree を何本でも
  持てる**のはこの命名の帰結

### 既存 Task の表示

どちらの一覧も、開いたときに **repo 内の全 worktree の Task を GitHub 参照で JOIN する**。
既に Task がある行は視覚的に区別する。これは「この対象には既に作業がある」ことの表示で、
選択の挙動は変えない。既存の作業へ戻る導線はサイドバーの Task 行が担う。

**issue の一覧は、既に Task があっても選択すれば新しい worktree を作る。** 同じ issue に
複数の worktree を持てる（別のアプローチを並行して試す用途）。

**PR の一覧だけは既存へ切り替える。** この経路は head ブランチをそのまま使うため、
2 本目を作ろうとしても git が拒否する。作れない以上、選択の行き先は既存への移動になる。

- closed な Task は同じ参照での再選択で蘇生する（PR の一覧のみ）
...
- **選択中の worktree は動かさない**。この経路の起点は人ではなく、人はそのとき別の worktree で
  ...
  作成（一覧経由 / 新規作成ボタン）は従来どおり作った worktree へ切り替える
- **この経路の Task は GitHub 参照を持たない**。参照に紐づく Task を作るのは一覧経由の作成
  だけで、切り出す側は「作って走らせる」ことを求めている
- **指示文は claude が起動するまでどこにも残らない**。作成の応答は「main が作れたか」だけを表し、
  ...
  セッションが指示文を保持するので、Task が持つ必要はない
...
- 母集団は repo プール全体の Task。アクティブ repo list で絞らない（list で絞ると
  「動いているのに出ない」Task が生まれる）
- 並びは最終活動の新しい順。最終活動はセッション稼働中ならその活動時刻、停止中なら
  セッションログの最後の記録時刻。セッション未起動の Task は最終活動を持たず末尾に置く。
