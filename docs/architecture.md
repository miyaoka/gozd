...
ただし state ファイルの**列挙値のフィールドに知らない値が入っていても破損とはみなさない**。
そのフィールドだけ default に倒し、ログを残す。state は channel をまたいで共有され
（「channel によるリソース分離」）、新しいビルドが足した値を古いビルドが読むことがあるため。
型そのものの違反（文字列のはずが数値など）は破損として扱う。
