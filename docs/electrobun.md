...
CLI cold start 時に、アプリ起動前に `$TMPDIR/orkis-stable-launch/{uuid}.json` へ `{ dir, file }` を書く。アプリは起動時に `readdirSync` + `readFileSync` で同期的に読み取り、処理済みファイルは `.claimed` にリネームする。TTL（30秒）超過のファイルは起動時に掃除する。読み取り失敗時は `Utils.showMessageBox` でエラーダイアログを表示する。

`resolveRepoRoot`（`git rev-parse --show-toplevel`）は CLI 側で実行する。アプリ側は同期処理のみで完結する。
