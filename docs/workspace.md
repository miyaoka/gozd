...
アプリ内に z-index による重ね順管理は持たない。

例外はトーストで、常にサーフェスより手前に留める。エラー通知の一次表示がパネルに埋もれると
失敗の可視性が落ちるため。メニューとモーダルは例外ではない（サーフェスの前面化はクリックで
起き、そのクリックがメニューの light dismiss を兼ねる。モーダルは backdrop が背後のクリックを
吸うため前面化の経路がない）。

実装は `shared/surface` に集約する。開閉も前面化もここを通す契約で、迂回すると「今どれが最前面か」
の判定が壊れる。順序が z-index ではなく show 順で決まる理由と、積み直しが見た目のコストを
持たない理由は同モジュールの docstring が SSOT。
