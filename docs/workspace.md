...
`gozd <path>` でプールの既存プロジェクトを開いたとき、それが表示中のリストに無ければ末尾へ追加する
（開いたのに何も見えない状態を作らない）。
...
- 最後に選択していた worktree
- サイドバーの表示（repo の階層 / 状態別）

**保存は snapshot の内容が実際に変化したときだけ発火する**。git status やセッションの一覧のように
...
- state が無い初回起動は、窓口だけが出たサイドバーで待機する
...
- 新規 worktree ボタン / Issue 経由 / エージェントからの作成要求: リモートの既定ブランチ起点
...
削除は **git の登録が解除された時点で完了**とし、実体が消えるのを待たない。worktree は依存ツリーや
ビルド成果物を抱えるため、実体の削除に掛かる時間はエントリ数に比例する。UI の応答をその量に
連動させない。

- **削除の完了はサイドバーから消えることを意味し、ディスクが空くことは意味しない**
- **失われて困るものを持つ worktree は拒否する**。変更のあるファイル・追跡外のファイル・submodule
  のいずれかがあれば消さず、ユーザーが強制削除を選んだときだけ消す
- **他のツールが lock した worktree も拒否する**。lock は別の持ち主が使用中である印で、強制削除を
  選んだときは lock も無視し、持ち主の作業場所ごと消す
- **拒否されたとき worktree は元の状態で残る**。登録も実体も削除前と同じで、再試行できる
- **消え切るまでの実体は、gozd のものと識別できる名前で残る**。worktree の一覧には現れない
...
| git status        | 全 worktree                    | 監視登録の成立、監視イベント、worktree 切替、Claude の状態遷移 |
...
| worktree / branch | 取得時のスナップショット       | 開いたとき、および該当 repo の構造変化                         |
...
- **ファイルツリーの表示だけは選択中 dir に閉じる**。表示していない dir のツリーを構築する必要が
  無いため
- **git status は worktree ごとに監視が取り、worktree 一覧の取得に連動させない**。一覧の取り直しの
  たびに worktree の数だけ作業ツリーを走査しない（[git.md](git.md)）
...
ウィンドウに同居する project は、**窓口**（[concierge.md](concierge.md)）と**プール**からなる。
窓口は gozd が常に 1 つ持つ project で、ツールバーの直下に固定で出す。プールはユーザーが開いた
project の集合で、state に保存する。どの project の dir かの判定・ファイル監視・PTY は、窓口と
プールの区別なく扱う。

プールの project は **リスト** 単位で表示する。リストはプールに対する名前付きビューで、表示する
部分集合と並び順を持つ。1 つの project は複数のリストに所属できる。**プール = 全リストの union が
不変条件**で、どのリストにも属さない不可視の project は存在しない。
...
- **トップツールバー**: ビューモードの切替、サイドバーの表示の切替、時計
- **窓口**: ツールバーの直下に固定する。リストの切替や編集モードで消えない
...
切り替えのたびに操作の起点が消える。端末単位の表示はビューモード（[terminal.md](terminal.md)）が担う。

サイドバーは、この project セクション一覧の代わりに、全 project のセッションを状態別に並べる表示へ
切り替えられる（[session.md](session.md) の「状態別の表示」）。リストバーはこの表示に効かないため出さない。
検索と詳細を伴う横断の一覧はダッシュボード（[session.md](session.md)）が担う。
...
- 配下のセッション行: その worktree で動いた Claude セッションを 1 セッション = 1 行で並べる。
  並びと表示は [session.md](session.md) の「サイドバー」

非 git project はセクションのヘッダ直下に同じセッション行を並べる。
...
ターミナルのスクロールバックは保存しない。代わりに Claude Code のセッションそのものを再開する。
再開の起点は Claude Code のセッションログで、gozd は専用のストアを持たない。
...
resume が走るのは **端末の開いていないセッション行を明示的にクリックした経路だけ**。
...
**resume 可否を事前に検証しない**。明示クリックを尊重し、失敗したら zsh 初期化が素の起動に
フォールバックする。

### 端末とセッションの紐付け

端末とセッションの対応は開始 hook で結び、次の契機で解く。**PTY の終了そのものは契機にしない**。
...
紐付けを解いてもセッションは消えない。行は端末の開いていない側へ移る。

アプリの強制終了では終了処理が走らないため、**次回も resume できる状態のまま残る**（これが正しい
挙動）。

**セッションログの存在を先回りして確認しない**。再開に失敗したら zsh 初期化が素の起動に
フォールバックし、新しいセッションとして現れる。

紐付けの解除要求は PTY の識別子をキーに発火し、受け側は識別子からセッションを解決する。これにより
hook の到達順に左右されない。**解除の後に遅れて届いた開始通知は弾き、弾いた事象を
ログに残す**。

### 削除済み worktree のセッション復活

worktree を削除すると resume は届かなくなる。**cwd のパスが消える**ためで、Claude Code は cwd から
セッションログの置き場所を導くため、実体が無いと引き当てが成立しない。サイドバーも作業
ディレクトリの無いセッションを出さない。
...
- **復活**: worktree を作り直し、再開はサイドバーのクリックと同じ意図の受け渡しで駆動する。
  **専用の resume 経路を増やさない**
