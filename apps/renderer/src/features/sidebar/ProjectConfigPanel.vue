...
async function load() {
  const result = await tryCatch(request.projectConfigLoad());
  if (!result.ok) return;
  const text = result.value.worktreeSymlinks?.join("\n") ?? "";
...
async function save() {
  ...
  const result = await tryCatch(request.projectConfigSave({ worktreeSymlinks: symlinks }));
