...
- `devDependencies.shiki: 4.0.2` (pinned) — generator script (`scripts/generateExtensionLangMap.ts`) が build 時に使う `bundledLanguagesInfo` / `BundledLanguage` の参照元。**生成テーブルの決定性を確保するため pinned**。caret にすると workspace の他 package が新しい shiki を持ち込んだ瞬間に生成内容が変わり、git 差分が install タイミングに依存してしまう
