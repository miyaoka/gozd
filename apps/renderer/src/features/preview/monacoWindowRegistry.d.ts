 ...
 * `patches/monaco-editor@<version>.patch` が export を復元しており、この宣言はその patch とセット。
 * 型は VSCode 本体 (`src/vs/base/browser/dom.ts` / `window.ts`) から最小 surface を書き写す。
 *
 * `registerWindow` は registry のキーに `vscodeWindowId` を使い、未焼き込みのウィンドウを
 * 渡しても throw せず key `undefined` で登録してしまう (2 つ目の未焼き込みウィンドウは
 * 「登録済み」と誤判定され silent に無視される)。引数型を `CodeWindow` に狭めることで、
 * `ensureCodeWindow` を通っていないウィンドウを型検査で弾く。
 */
