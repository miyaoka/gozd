...
import type { DiffExpandedLine } from "@gozd/rpc";
...
import {
  ...
  lineNoWidth as computeLineNoWidth,
  splitIntoSections,
} from "./diffViewModel";
...
watch(
  ...
  async ([original, current, editable], _, onCleanup) => {
    ...
    state.value = {
      ...
      ranges: buildResult.value.ranges,
      oldTotal,
      newTotal,
    };
  },
...
const lineNoWidth = computed(() => {
  ...
  return computeLineNoWidth(s.oldTotal, s.newTotal);
});
...
/** unified の描画行。展開済みバーの置換と tokens / 行内 range の埋め込みは diffViewModel が持つ */
const renderRows = computed(() => {
  const s = state.value;
  if (s.kind !== "success") return [];
  return buildUnifiedRenderRows(
    s.baseItems,
    originalTokens.value,
    currentTokens.value,
    s.ranges,
    expansions.value,
  );
...
/** split の描画行。展開済みバーの置換と tokens / 行内 range の埋め込みは diffViewModel が持つ */
const splitRenderRows = computed(() => {
  const s = state.value;
  if (s.kind !== "success") return [];
  return buildSplitRenderRows(
    s.baseSplitItems,
    originalTokens.value,
    currentTokens.value,
    s.ranges,
...
const unifiedItems = computed<UnifiedItem[]>(() => splitIntoSections(renderRows.value));

const splitItems = computed<SplitItem[]>(() => splitIntoSections(splitRenderRows.value));
