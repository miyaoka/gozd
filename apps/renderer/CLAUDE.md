...
- 失敗経路の `logEvent` は detail に原因を載せる。発火・抑止・失敗を 1 本の時系列で持つのはイベントログだけで、原因が別経路にあると頻度と原因を突き合わせられない
...
`<doc>` はそのコンポーネントの **目的・責務・非自明な設計判断（なぜそうしているか）** を書く。実装は書かない。

書く:

- 冒頭にコンポーネントの概要（責務）を一文で書く
- 非自明な設計判断の「なぜ」（例: なぜ store の signal 経由なのか、なぜ sticky を scroll 内に置くのか）
- ブラウザ挙動・アルゴリズム不変条件・レイヤー間の契約など、コードを読んでも分からないこと

SFC 固有で書かないもの:

- Props / emit など実装から読み取れる情報
- ファイル名から自明なタイトル（`# ComponentName`）

### `docs/` を指すときはファイル名で書く

機能ドキュメントを参照するときは `docs/preview.md` のようにファイル名だけを書き、相対パスの
リンクにしない。節を指すなら `docs/filer.md` の「symlink の表示」のように節名を書く。

**相対パスは参照先の事実ではなく、参照元がどこに置かれているかで決まる値**であり、ファイルを
移動しただけで壊れる。壊れても typecheck / lint には掛からないため、誤りが残り続ける。
`docs/` は repo root 直下に固定でファイル名も機能名と一致するため、名前だけで到達できる。
