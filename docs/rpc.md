...
- **購読者が 1 人も居ない push は捨てる**。購読は mount で張られるため、renderer の
  構築中に届いた push は誰にも渡らない。捨ててよいのは「mount で pull、変化で push」
  （[architecture.md](architecture.md)）に乗る payload だけで、pull で取り直せる
- **pull の相手を持たない payload は、捨てたことを観察ログに残す**。取りこぼしても
  回復できないため、失われた事実だけは残す。対象の type はバスが保持する
