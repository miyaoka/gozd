...
- snapshot mode 中、NavigatorPane の Filer ヘッダーに選択中コミットの日時（絶対時刻）と "Now" ボタンを表示する。"Now" は `gitGraphStore.select(UNCOMMITTED_HASH)` を呼び、GitGraphPane の Working Tree 行クリックと同じ経路で working tree モードに戻す。「今 snapshot を見ている」ことと「working tree へ戻る」手段を、GitGraphPane 側を操作せず filer 側単独で完結させる
