...
<template>
  ...
  >
    <div class="flex shrink-0 items-center gap-1.5 border-b border-border px-3 py-1.5">
      <span class="icon-[lucide--git-branch] size-4 text-foreground-muted" />
      <span class="text-xs font-semibold text-foreground-muted">Changes</span>
      <span v-if="changesStore.fileChanges.length > 0" class="text-xs text-foreground-subtle"
      ...
      <button
        ...
        :class="prDiffToggle.isOn ? 'text-info' : 'text-foreground-subtle hover:text-foreground'"
        :title="
      ...
      <button
        ...
        :class="[
          summaryStore.enabled ? 'text-info' : 'text-foreground-subtle hover:text-foreground',
    ...
    <div v-if="changesStore.loading" class="flex-1 overflow-y-auto p-2">
      <div class="text-xs text-foreground-subtle">Loading...</div>
    </div>

    <div v-else-if="tree.length === 0" class="flex-1 overflow-y-auto p-2">
      <div class="text-xs text-foreground-subtle">No changes</div>
    </div>
