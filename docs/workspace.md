...
プロジェクトや worktree を切り替えて非表示になっても、以下のビュー状態は破棄せず保持する:
...
一方、プレビュー対象の選択ファイルは worktree 切替時に明示的にクリアする。worktree 横断で `useWorktreeStore.selection` 1 つだけを保持し、`setOpen` で dir が変わったタイミングで undefined にリセットする。複数 worktree を行き来したときに、別 worktree のファイル選択が裏で残り続けるのを避ける狙い。

### workspace-scoped な状態管理（部分実装）

> [!NOTE]
> ターミナル（`useTerminalStore.layoutsByDir`）は dir ごとの保持を実装済み。ファイラー展開状態・プレビュー選択はまだ単数 state で、選択 dir 切替時にリセット / 再取得する設計。
