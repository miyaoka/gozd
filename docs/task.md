...
弾く場所は一覧で違う。issue の一覧は受理を通し、作らずに実行中である旨を通知する。PR の一覧は
ダイアログが受理そのものを落とす。
...
- **GitHub 参照を指定しても、その参照に既に worktree があるかは見ない**。同じ番号を何度
  投入しても、そのつど新しい worktree を作る。切り出す側は「作って走らせる」ことを求めて
  おり、既存への移動を求めていない。**この経路は参照の種別に関わらずブランチ名を
  タイムスタンプで作る**ため、PR を指定しても 2 本目が作れる（一覧経由との違い）
