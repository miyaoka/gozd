...
(JSONL が在れば `~/.claude/projects/<encoded>/`、未生成なら `~/.claude/projects/` 親)
を `rpcFsWatch` で監視し、`fsChange` push を受けたら debounce して再読込する。再読込は
`loading` を立てず `sessions` を差し替えるだけのサイレント更新で、`SessionLogTranscript`
の remount とスクロール状態リセットを避ける。context が undefined になった / dialog が
アンマウントされた時点で composable が `rpcFsUnwatch` を発射する。worktree 外のログ
なので filer の app-scope watch には乗らず、composable が watch ライフサイクルを所有する。
</doc>
