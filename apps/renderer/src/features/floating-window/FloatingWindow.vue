...
- ドラッグ移動はヘッダの pointerdown で開始し、window レベルの pointermove / pointerup
  ...
  drag handoff のため: pin 元ヘッダのドラッグで pin する経路では、掴んでいた要素が pin と
  同時に消えて capture が死ぬ。window listener なら「mount 直後に dragState を立てるだけ」で
  同じ pointer のドラッグを途切れず継続できる
- `handoff` prop (pin 元が consumer store の takeHandoff() で消費した引き継ぎ) があれば
  pointerdown なしでドラッグ中状態から始まる (pin 元を掴んだ手がそのままウィンドウを
  掴んでいる状態)
- x / y prop は「ユーザーが望んだ位置」で、描画時に CSS の `min()` / `clamp()` で
...
- 初期サイズは pin 元の本文実測 (bodyWidth / bodyHeight prop) に、mount 時に実測した
  自ヘッダ高 + border を足して総サイズへ換算する (本文サイズを不変量にする理由は
  useFloatingWindows の doc 参照)。`:style` にはバインドせず mount 時に一度だけ inline
  style へ書く。Vue の style patch はバインドオブジェクトの全キーを毎パッチ再適用するため、
...
- リサイズは 8 方位の不可視ハンドル + pointer capture (ネイティブ `resize: both` は仕様で
  ...
  pin 元からの drag handoff が存在せず、capture を避ける理由がないため。左/上辺は
  反対辺アンカーで width / height と x / y を同時更新する (位置は move emit 経由)。
...
- window 内どこかを pointerdown した時点で activate を emit する (最前面化の z 更新は
  consumer store 側)
- 最小サイズ (min-h-16 / min-w-64) は掴み代の確保が目的。総高さ (本文実測 + 自ヘッダ +
  border) はヘッダ差分のぶん常に pin 元 box より高く、1 行コンテンツ等で総高さが下限を
