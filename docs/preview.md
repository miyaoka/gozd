...
- Filer 経由で PR diff に含まれない (merge-base 起点で無変更の) ファイルを選択したケースは、`orderedFileChanges` に該当 change が無いため working tree の中身を `rpcFsReadFile` だけで Current に出して救済する。commit モードの「範囲外ファイル救済」(上表の「変更なし」行) と対称。空表示に倒すと Filer で見えるファイルが PR モード中だけ読めなくなる非対称が生じるため。from (base) 側を撃たないので存在しないパスへの `git show` による bogus stderr も出ない
