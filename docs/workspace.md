...
- **WebView と titlebar の位置関係**: top safe area の扱いは channel で分岐する（`ignoresSafeArea(.container, edges: GozdApp.isDev ? [] : .top)`）。stable は WebView を titlebar 下まで延ばし、Liquid Glass titlebar が WebView の背景輝度をサンプルしてコンテンツと同調した dark になる。respect に変えると glass が WebView 背後の `.background(Color.black)` を拾い titlebar だけ黒く浮く（実機確認済み）。dev は `toolbarBackground` 塗り（後述）で titlebar 高が不透明な top inset になるため respect する。無視すると WebView 上端が不透明 titlebar の真下に隠れる（実機確認済み）。なお WebKit `WebPage` は macOS で titlebar 高を `env(safe-area-inset-top)` へ伝えない（`viewport-fit=cover` を宣言しても実測 0）ため、「titlebar 背景を hidden にして renderer に titlebar 領域を描かせる」公式パターン（Destination Video）は reservation が取れず gozd では成立しない
- **WebView 背景**: `.webViewContentBackground(.hidden)` で WebView 自身の opaque white default を消し、`.background(Color.black)` を敷いて dark に倒す
- **Color scheme**: `ContentView` に `.preferredColorScheme(.dark)` を当て、NSAppearance 経由で titlebar / traffic light / toolbar item の文字色等も dark mode 仕様に揃える
- **dev channel の識別色**: dev 起動時のみ `.toolbarBackground(devTitlebarTint, for: .windowToolbar)` で titlebar を彩度を抑えた緑に塗る（「insiders ビルド = 緑」の VS Code Insiders 慣習。`GozdApp.swift`）。stable は素のまま。Liquid Glass 公式ガイドは toolbar への custom background を非推奨とするが、dev channel 限定の開発者向け識別として許容する判断
