...
- `build:app` が `buildVersion`（Info.plist の `CFBundleVersion`）に HEAD のコミット日時 + hash を
  注入する。バージョン番号は安定版まで管理しない（0.0.0 固定）ため、About パネルの括弧内が
  ビルド識別の唯一の手段。未コミット変更を含むビルドは hash に `-dirty` が付き、表示 hash と
  `.app` の中身の不一致が自己申告される。electron-builder ステップでの注入なので packaged（stable）にしか値が
  存在せず、「dev には出さない」が配管なしで保証される。dev はビルド元 worktree でコードが
  見えるため識別情報を焼き込まない
...
未パッケージ（`electron .`）は `dev-<worktree hash>`（electronRoot の realpath から導出）。
判定は `gozdEnv.isPackaged`（`__dirname` が `process.resourcesPath` 配下かどうか）。
...
専用経路を持たない。画像 / SVG もテキストと同じ read RPC（`fsReadFile` / `gitShowFile` 等）で
`WireBytes`（生 bytes）として運び、renderer が Blob → ObjectURL に変換して `<img>` に渡す。
旧 `gozd-file://` protocol（`<img>` への raw bytes 直配信 scheme）は、structured clone ワイヤへの
移行で「JSON string はバイナリを保持できない」という前提ごと廃止した。
