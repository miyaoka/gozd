<doc lang="md">
undock されたコンテンツ 1 件のフローティングウィンドウの汎用シェル。

ドラッグ移動 / 8 方位リサイズ / drag handoff / ビューポート内クランプ / 初期サイズ換算を
担い、ヘッダ内容と本文は slot で consumer (UndockedLogWindow / UndockedPreviewWindow) が
...
interface Props {
  ...
  bodyWidth: number;
  bodyHeight: number;
  /** 外部からの close 要求 epoch (FloatingWindowState.closeRequested)。 */
