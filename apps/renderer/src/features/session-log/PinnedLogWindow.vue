...
- 本文の配色 / markdown 描画は TerminalSessionPreview の全文 popover と同じ規律
  (assistant のみ MarkdownBody。user 投稿は素のテキスト前提で markdown 解釈しない)
</doc>
...
<template>
  ...
  <section
    ref="root"
    class="fixed flex max-h-[80vh] min-h-40 max-w-[90vw] min-w-64 resize flex-col overflow-hidden rounded-md border border-border-strong bg-background shadow-xl"
    :style="{
  ...
  >
    ...
    >
      <div
        v-if="log.kind === 'assistant'"
        class="_pinned-assistant px-3 py-2 text-chat-incoming-text [--color-foreground-low:var(--color-chat-incoming-text-low)] [--color-foreground:var(--color-chat-incoming-text)] [--md-code-bg:transparent]"
      >
        <MarkdownBody :content="log.text" />
      </div>
...
<style scoped>
/* MarkdownBody は `_markdown-body :deep(code)` で inline code を foreground に染めるため、
   chat 配色の本文で code だけ紫にするには CSS 変数 override では足りない。
   TerminalSessionPreview の `_preview-assistant` と同じ specificity 持ち上げ。 */
._pinned-assistant :deep(code) {
