...
RPC 型は `packages/proto/` の `.proto` を SSOT に置き、buf（ts-proto）で TS を生成する。

- ts-proto → `packages/proto-ts/src/generated/`（`@gozd/proto` として renderer / electron が import）
- 生成物は git に commit しない。`packages/proto/` の `prepare` script（`buf generate`）が生成する
- `.proto` 編集時の再生成は手動: `pnpm --filter @gozd/proto-schema build`（`node_modules` が
  up-to-date だと pnpm の auto-install は発火しないため）

> [!NOTE]
> proto は Swift 並走期の二言語型共有の名残。シェルが TypeScript に一本化されたため、
> 共有 TS 型パッケージへの置き換え（proto 廃止）が次の独立ステップ（issue #895）。
> proto3 `oneof` は discriminated union に自然に戻る。
...
- `electronDist` で pnpm 展開済みの Electron dist を使う（zip ダウンロード回避）
...
ワイヤは NDJSON 1 行の proto3 JSON `ClientMessage`。launch dir の channel は
...
- **request**（応答あり）: renderer → main。`ipcRenderer.invoke("rpc:request", path, bodyJson)` →
  `rpcDispatcher.ts` のルート表 → 実装は `routes.ts`。ワイヤは proto3 JSON
...
raw bytes を返す。proto3 `string` がバイナリを保持できないため、`<img>` 経路だけ別 scheme に
分けている（proto 廃止時に経路再設計の余地あり）。
...
（改行区切り JSON）。1 行が proto3 JSON でエンコードされた `ClientMessage`（`oneof` で
`OpenMessage` / `HookMessage` を切り替え）。
...
`~/.local/state/gozd/` に proto3 JSON で保存する。どちらも dev / stable で共有する。
...
- `AppState` の save は既存ファイルを raw dict として読み shallow merge して未知 top-level キーを
  保持する（既知キーは merge 前に削除。proto3 JSON が空 repeated を省略する性質による
  「最後の repo を消したのに古い `sidebarRepos` が残る」事故を防ぐ）
- `TaskStore`（`tasks.json`）の load は parse 失敗時に**空オブジェクトで上書き save** する
  （後方互換を作らない規約。主データを JOIN する立場のため load 経路から throw を伝播させない）。
  stderr に reinit ログを残して観察可能性を保つ

### 新しい永続化データを追加するパターン

- `packages/proto/gozd/v1/*.proto` に request スキーマを追加し、
  `pnpm --filter @gozd/proto-schema build` で生成物を更新する
- `apps/electron/src/` にファイル I/O モジュールを作成する（`stores.ts` / `taskStore.ts` が参考実装）
- `apps/electron/src/routes.ts` に handler を登録する
- renderer 側は feature ごとの `rpc.ts` に `rpcXxx()` 関数を追加し、`shared/rpc` の
  `rpc(path, req, RequestType, ResponseType)` でラップする
