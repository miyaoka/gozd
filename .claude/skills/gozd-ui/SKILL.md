...
### 外部 theme 由来の動的色 (inline style) は SSOT 例外として許容

`@theme` で表現できない **外部 system 由来の動的色** (Shiki syntax highlight / iTerm2-Color-Schemes
terminal theme 等) を inline style (`:style="{ color: token.color }"` / `:style="{ backgroundColor:
currentTheme.background }"`) で渡す形は許容する。理由: 外部 theme は OKLCH の semantic token を
知らず hex を返すため、token 化するには theme 自体を gozd 内で再定義する必要があり、外部 system
への追従が崩れる。

許容範囲は **以下 2 経路に限定**:

- Shiki theme 由来の syntax highlight color (`DiffPreview.vue` / `CodePreview.vue` の `token.color`)
- iTerm2-Color-Schemes 由来の terminal theme color (`TerminalLeaf.vue` の `currentTheme.background`)

自前 UI 色 (intent / surface / foreground / border) を inline style で渡すのは禁止 — それらは
必ず `@theme` token + Tailwind utility / `var(--color-*)` で書く。
