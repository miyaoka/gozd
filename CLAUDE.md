...
| RPC スキーマ       | Protocol Buffers（`.proto` SSOT、ts-proto。共有 TS 型化で廃止予定）         |
...
| `packages/proto`              | `.proto` SSOT。buf（ts-proto）で TS を生成                                                                                           |
| `packages/proto-ts`           | ts-proto 生成物。`@gozd/proto` として renderer / electron が import                                                                  |
