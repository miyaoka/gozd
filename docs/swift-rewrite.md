...
- xterm.js は CDN ロードか vendor。renderer 側の `useRpc.ts` 差し替えだけで既存 Vue 資産が動く
...
- **マルチリポ対応の RPC スキーマ** — issue #310 のステートレス化を `packages/rpc/` の型に反映する具体形
...
| 経路                           | 方式                                                                                                                             |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| Vue → Swift（request）         | `fetch("gozd-rpc://<name>", { method: "POST", body: JSON })` → `URLSchemeHandler.reply(for:)` が `HTTPURLResponse + JSON` を返す |
| Vue → Swift（fire-and-forget） | 同上で `204 No Content` を返す                                                                                                   |
| Swift → Vue                    | `WebPage.callJavaScript("window.__gozdReceive(type, payload)", arguments: …)`                                                    |
...
既存の `apps/renderer/src/shared/rpc/useRpc.ts` の内部実装だけ差し替えて、外部 API（`request`, `send`, `on`）は完全互換に保つ。feature コードは無変更。
...
| フェーズ    | 検証ゴール                                                                                                          |
| ----------- | ------------------------------------------------------------------------------------------------------------------- |
| **Phase 0** | `apps/native/` を新規作成。空の SwiftUI `Window` + `WebView(WebPage())` + `URLSchemeHandler` で echo RPC が往復する |
| **Phase 1** | バックエンドコア: PTY / FSWatcher / NWListener。`apps/desktop/` と同等の機能を Swift で再実装                       |
| **Phase 2** | Git ops + 状態永続化 + Claude hooks。RPC スキーマを既存 `packages/rpc/` の型と整合させる                            |
| **Phase 3** | Vue renderer 接続。`useRpc.ts` を Swift ブリッジ向けに差し替え。シングルウィンドウ・マルチリポ対応（issue #310）    |
| **Phase 4** | 配布: Xcode Archive + Notarize、CLI バンドル、`gozd` シェルラッパー差し替え、Sparkle 検討                           |
| **Phase 5** | `apps/desktop/` と Electrobun 関連の依存（`pnpm-workspace.yaml`、`package.json` 等）を削除                          |
