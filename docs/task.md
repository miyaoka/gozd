...
- **GitHub 参照を指定したとき、その参照の Task が生きている worktree に既にあれば作らずに
  失敗を返す**。UI は既存 worktree への切り替えに倒すが、CLI には切り替える画面が無いため、
  既存の置き場所を添えて失敗として返す。同じ番号を続けて投入しても増えない。判定集合は UI と
  揃えるため、worktree を消した後の再投入は新しく作れる
- **指示文は claude が起動するまでどこにも残らない**。作成の応答は「main が作れたか」だけを表し、
  指示文の受け渡しは後段の UI 反映が担う。起動までのどこかで失敗したら、**失敗の通知に指示文の
  本文を添える** — 添えないと何が失われたか分からず、手で渡し直せない。起動した後は Claude の
