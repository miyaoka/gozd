...
async function load() {
  ...
  if (!result.ok) {
    notify.error("Failed to load project settings", result.error);
    return;
  }
...
async function save() {
  ...
  if (!result.ok) {
    notify.error("Failed to save project settings", result.error);
    return;
  }
    ...
    <details ref="detailsRef" @toggle="handleToggle">
      ...
      <div class="mx-2 mt-1 mb-2">
        ...
        <div class="mt-1 flex justify-end gap-1">
          ...
          <button
            class="rounded-sm bg-primary px-2 py-1 text-xs text-foreground hover:bg-primary-hover disabled:bg-element disabled:text-foreground-muted disabled:hover:bg-element"
