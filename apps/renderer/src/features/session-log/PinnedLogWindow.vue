...
- 初期サイズは pin 元 popover の本文実測 (store の bodyWidth / bodyHeight) に、mount 後に
  実測した自ヘッダ高 + border を足して総サイズへ換算する。pin 元 popover のヘッダ
  (pin ボタン 1 行) とこのウィンドウのヘッダ (repo + タイトル 2 段) は高さが違うため、
  総高さを引き継ぐと増えたヘッダ分だけ本文が食われて切れる。本文サイズを不変量に
  すれば、ヘッダの行数 (repo 未解決で 1 段になる場合も) に関わらず本文が pin 元と同寸で
  残る。`:style` にはバインドせず mount 時に一度だけ inline style へ書く。Vue の style
  patch はバインドオブジェクトの全キーを毎パッチ再適用するため、バインドするとドラッグ
  (left / top 更新) のたびに width / height が初期値で再セットされ、native resize
  (`resize: both`) が書いた値を巻き戻してしまう。mount 後のサイズ SSOT はブラウザが書く
  inline width/height で、Vue は以後このプロパティに触らない
...
// を足して border-box の総サイズへ換算する (doc 参照)。
onMounted(() => {
