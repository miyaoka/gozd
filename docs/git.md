...
- **`scheduleLoadLog` (事前防衛)**: push 由来 handler (`branchChange` / `remoteRefsChange` / `fsWatchReady` / `gitStatusChange` の scroll 不要経路) はこれを呼ぶ。in-flight な `loadLog` があれば 1 bit の pending flag に畳み、完了後に trailing 1 fetch だけ追加発射する。burst N 発火を最大 2 fetch (in-flight + trailing) に集約する
- **`loadLogGen` (事後防衛)**: 明示 trigger (worktree 切替 / firstParentOnly / `headChanged` 経路) は scroll 制御のため `await loadLog()` で fresh fetch を起動する。これと burst の trailing fetch が並行することはあり得るが、世代管理で古いレスポンスは捨てる
