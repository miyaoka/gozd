...
const branchLabel = computed(() => props.wt.branch ?? "(detached)");
...
function resolveStateKey(status: ClaudeStatus | undefined): StateKey {
...
const tasksWithStatus = computed<TaskWithStatus[]>(() => {
  const list = props.wt.tasks.map<TaskWithStatus>((task) => {
    ...
    return {
      ...
      status,
      ptyId,
      stateKey: resolveStateKey(status),
