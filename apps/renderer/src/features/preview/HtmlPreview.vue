...
sandbox を外すと iframe 内 JS が renderer と同 origin (`gozd-app://localhost` / dev origin) で動き、
`gozd-rpc://` の CORS allowlist を通過して任意ファイル bytes を回収できてしまう。

`sandbox=""` で origin を opaque 化すると:

- `<script>` は実行されない (静的 HTML + CSS のみ描画)
- 仮に scripts を許可しても opaque origin は RPC allowlist に含まれず構造的に RPC を叩けない
