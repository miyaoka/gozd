...
- **信号機ボタン**: macOS が web コンテンツの上に合成する。位置は main の `trafficLightPosition` がタイトルバー高さ（renderer の `--titlebar-height` と同値契約）の垂直中央に合わせる。fullscreen ではボタンが消えるが左の逃げ幅は残す（cosmetic のみ、追従レイアウトは未実装）
