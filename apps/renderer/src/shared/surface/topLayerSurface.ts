 ...
 * したがって前面化は一度閉じて開き直すしか手段がなく、開閉・前面化・close の宛先
 ...
 *
 * 積み直しは `display: none` を経由する形で行う。破棄された box のスクロール位置が再表示時に
 * 復元されるのはこの経路だけで、要素を DOM から取り外して挿し直す形は top layer の順序としては
 * 同じ結果を出すが、中身の読み位置を巻き戻す。一方 top layer の順序はドキュメントの状態であって
 * レンダリングの結果ではないため、スタイル再計算を待たずに変わる。
 *
 * hide と show の間のサーフェスは `display: none` で focusable ではない。この区間へ出した focus は
 * 例外も出さずに何も起こさないため、操作の順序に制約が掛かる (surfaceStack の `planRaise` /
 * `planShow`)。
 *
 * ## フォーカスの行き先
 *
