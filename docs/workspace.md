...
- **タイトル表示**: 現在 active な repo / worktree を `"repoName · branchName"` 形式で表示。renderer 側 `useTitleContextSync` composable が `repoStore.selectedRepo` + `selectedDir` を watch し、`rpcWindowSetTitleContext` で main に送り、main は `BrowserWindow.setTitle` に反映する
