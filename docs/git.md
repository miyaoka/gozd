...
| `gitStatusChange`  | per-wt の `index` / HEAD、common の `refs/remotes/*`、作業ツリー側のファイル変更 | useGitStatusSync (全 worktree の ahead/behind 反映)、FilerPane / GitGraphPane (active の HEAD / branchHead / upstream 変化で `loadLog`)            |
...
### `refs/remotes/*` / `packed-refs` の二重発火を許容する設計判断

`refs/remotes/*` は `gitStatusChange` + `remoteRefsChange` の **両方** を発火する。`packed-refs` は `branchChange` + `gitStatusChange` + `remoteRefsChange` の **3 つ** を発火する。これにより、active worktree の current branch を `git push` した場合、GitGraphPane で `loadLog` が短時間に最大 2 回 (current branch ref で `upstreamChanged` ＋ `remoteRefsChange`)、`packed-refs` 経由なら最大 3 回発火する。
...
- `loadLogGen` の世代管理 (`GitGraphPane.vue` `loadLog`) で in-flight な古いレスポンスは捨てられるため UI 状態は壊れない
- 観察可能性ログには重複した `rpcGitLog` 発射が乗る。auto-fetch (180s) × N repo × M worktree の運用環境で問題化したら、`loadLog` 側に in-flight coalescing を入れる選択肢が残されている
