...
- **PTY の起動要求で渡した環境変数は、gozd が保証する項目を上書きしない**。ハイパーリンクや
  カラー対応の宣言は、指定が無いときだけ埋まる
...
- 選択中 worktree のタイトルが更新されると、紐づく Task のタイトルを同期する。
  **既にタイトルがある Task は上書きしない**（[task.md](task.md) のタイトル 3 レイヤ）
...
ターミナルはコア内部でスクロール位置を管理するためこの問題が起きないが、**組み込みのエミュレータでは
コア内部を変更できないため外部から補正する**。

主要ターミナルはいずれも「スクロールバック行数」または「論理行インデックス」をコアの内部状態として
持ち、行が追加されるたびに加算している。**外部の callback で補正するパターンはどのターミナルにも
存在しない**。

そこで採るのが **意図 + アンカー** モデル。

```text
意図 = bottom | anchored(アンカー)

書き込み前:
  最下部にいる or 代替バッファ → 意図 = bottom
  スクロールバック中          → アンカーを打って 意図 = anchored
...
アンカーは行の追加・削除に追従する。復元は描画確定のタイミングで集約するため、**高頻度の書き込みでも
復元は 1 回に収束する**。

### 検討して採らなかった方式

- **スクロール位置の生値を共有変数に持つ**: 非同期の書き込みキューとスナップショットの対応が崩れる
- **生値をクロージャに捕捉する**: コールバック実行時に古いスナップショットでユーザー操作を踏み潰す
- **世代番号で管理する**: 主要ターミナルのどれにも存在しないパターン

### リサイズ時

アンカーによる復元を試みるが、TUI アプリの再描画でずれる場合がある。**他のターミナルでもリサイズ時は
最下部にリセットされるため許容する**。
