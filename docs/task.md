...
**候補ピックアップ (2 つ目の優先順位) で hidden な task を外す**のは、terminal close 済みの ghRef task に、別経路で起動した素 claude (新 sid) が偶発的に取り憑くのを構造的に防ぐため。sid 不一致での蘇生経路は「PR/issue picker での再選択 → `TaskStore.add` の upsert」に一本化される。sid 一致の蘇生は本ロジックの 1 つ目の優先順位が担う。
...
| ターミナル close (PTY 終了)              | `detachSession`: sessionId 切り離し。`ghRef` 無しなら task 削除。`ghRef` ありなら `hidden=true`                                                                                                                | `claude --resume <同 sid>` で SessionStart hook 着弾 → `attachSession` が sessionID 一致経路で `hidden=false` に倒す。または PR/issue picker で同じ `ghRef` を再選択 → `add` の upsert で `hidden=false` |
