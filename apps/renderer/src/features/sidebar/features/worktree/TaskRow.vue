...
live PTY 無し（resumable）は status が無いため `task.createdAt` にフォールバックする
（永続化された session の lastActivityAt 復元は別 issue で対応）。
</doc>
...
/**
 * 相対時刻の基準時刻。status があれば `lastActivityAt`、無ければ `task.createdAt`。
 * createdAt のパースに失敗（NaN）したら時刻表示なし。
 */
const baseTime = computed<number | undefined>(() => {
  if (props.status !== undefined) return props.status.lastActivityAt;
  const created = Date.parse(props.task.createdAt);
