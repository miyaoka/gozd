...
>
> 例外: application menu の accelerator に bind されたキー (Cmd+Q の Quit、Cmd+H の Hide、Cmd+M の Minimize 等) は JS handler に届く前に処理されるため bind 不可。menu 構成は `apps/electron/src/menu.ts`（role ベース）。`defaultKeyBindings.json` は Cmd+W を `terminal.closePane` に割り当てているが、role: fileMenu の Close Window accelerator (Cmd+W) と競合する可能性があり、Electron 化後の実挙動は要検証。
