...
全チェックはルートの `pnpm run typecheck:all` / `pnpm run lint:all` / `pnpm run test:all` / `pnpm run build:all` / `pnpm run knip` で行う（CI の `code_validation.yml` matrix と同じ 5 種）。pnpm 11 は `pnpm run` 実行時に node_modules を自動インストールするため、事前の手動 install は不要。
