...
`-mt-2` は section の gap-2 を相殺し、除去/挿入の瞬間に 8px の段差が出ないようにする。
</doc>
...
<template>
  ...
  >
    ...
    <Transition
      enter-from-class="h-0 -mt-2"
      enter-active-class="overflow-hidden transition-[height,margin-top] duration-200 ease-out"
      leave-active-class="overflow-hidden transition-[height,margin-top] duration-200 ease-out"
      leave-to-class="h-0 -mt-2"
    >
      <div
        v-if="isGitRepo && !visiblyCollapsed"
        class="flex flex-col gap-2 [interpolate-size:allow-keywords]"
      >
        <WtCard
