...
- `gitStatusChange`: `git status --porcelain=v2 --branch` の結果をプッシュ通知（300ms デバウンス）。HEAD ハッシュも同時に取得
...
- `MainLayout` が `selectedPath` の変更を watch し、Preview 自動オープンと合わせて `NavigatorPane` 経由で `reveal` を呼び出す

## git status の色分け

`git status --porcelain=v2` のステータスコード（XY の2文字）から変更種別を判定する。worktree 側（Y）を優先し、なければ index 側（X）を使う。
