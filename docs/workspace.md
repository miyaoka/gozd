...
削除は **git の登録を解除した時点で完了**とし、実体の unlink は待たない。worktree は依存ツリーや
ビルド成果物を抱えるため、実体の削除に掛かる時間はエントリ数に比例する。UI の応答をその量に
連動させない。

- **拒否の条件は git と同じ**。変更のあるファイル・追跡外のファイル・展開済み submodule を持つ
  worktree は拒否し、ユーザーが強制削除を選んだときだけ消す。実体を先に切り離す以上、この判定を
  git に委ねることはできない
- **git が拒否したら元の状態へ戻す**。ロックされた worktree のように gozd 側では判定しないものが
  残るため、切り離した実体は復帰させる
