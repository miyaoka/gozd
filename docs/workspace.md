...
- **claude ビュー連動フィルタ**: ターミナルが claude タイル表示のとき、サイドバーも Claude セッションが
  ...
  常に一致する。対象 repo は折りたたみ状態を無視して展開する（タイルは collapse と無関係に出るため、
...
worktree を削除すると上記の自動 resume は届かなくなる。理由は 2 つ:
...
- **task が消える**。`removeByWorktree` が worktree 削除時に該当 task を tasks.json から消すため、`resumableSessionIds` の対象集合にも載らない
