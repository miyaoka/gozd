...
`pnpm dev` は `apps/native/scripts/build-dev-app.sh` で **dev 専用 `.app`**（dock icon が dev カラー、タイトルが `gozd (dev)`）を生成し、renderer は Vite dev server（`http://localhost:16873`、`GOZD_DEV_VITE_URL` で指定）から読み込む。Vite default の 5173 ではなく gozd 固有ポートを使い、かつ `vite.config.ts` で `strictPort: true` を指定する。理由は別 worktree からの二重 `pnpm dev` / 別 Vite アプリと衝突したときに fallback で別ポートを掴むと、Swift `.app` は `GOZD_DEV_VITE_URL` 固定なので先発の Vite に繋がって「別 worktree のはずなのに先発の内容が表示される」サイレント事故になるため。`pnpm build` は `build-app.sh` で stable 版 `.app` を生成し、renderer はバンドル内 `views/main/index.html` を `gozd-app://` URLSchemeHandler 経由でロードする。
...
- native の dev スクリプトは `GOZD_DEV_PROJECT_ROOT=$(repo root)` `GOZD_DEV_VITE_URL=http://localhost:16873` を渡して `apps/native/scripts/build-dev-app.sh` を実行する
