...
- **訪問は選択と独立する**。選択中の worktree を切り替えれば訪問するが、選択を動かさずに
  訪問する経路もある（[session.md](session.md) の「エージェントから worktree を作る」）。表示されて
...
| OSC | 用途               | 使い道                                |
| --- | ------------------ | ------------------------------------- |
| 0/2 | ウィンドウタイトル | セッションタイトル、Claude の状態検出 |
...
- Claude が動いている端末のタイトルは、そのセッションの表示タイトルの第一候補になる
  （[session.md](session.md) の「タイトル」）
- **端末ヘッダに生の OSC タイトルや cwd は表示しない**。Claude セッションが動いている端末だけ、
  サイドバーと同一の見た目で「repo 名」と「セッションタイトル」を 2 段で表示する（素の端末には何も
  出さない）
...
- **URL の外側は中身によらず落とす**。終端の約物より後ろに約物でない文字（英数字など）が
  続いていても URL には含めない。`https://example.com/a（補足）` では `（補足）` が外側にあたる
