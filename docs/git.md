...
| RPC                     | 用途                                                              | 呼ばれる場面                                                                                                           |
| ----------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `rpcGitStatus`          | working tree status + HEAD + upstream + 変更ファイル mtime 最大値 | 初回 load、`gitStatusChange` push 後の再 fetch                                                                         |
...
- `StatusFull.==` の比較対象には `latestMtime` (= 変更ファイルの mtime 最大値、秒粒度) を含める。既存差分ファイルを再保存しただけのケースで Working Tree 行の date 列を更新するため、mtime 変化は push 対象として扱う。秒粒度なので、同一秒内の連続保存は mtime が同値となり自然に dedup される (push 流量は最大 1 push / 秒 / dir)。gitignored ファイルは `statuses` に入らないので mtime 集計の対象外で、上の typecheck 連射 dedup を壊さない
