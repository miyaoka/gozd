...
レイアウトフローから分離して重なる面を **サーフェス** と呼ぶ。preview / server list / event log の
右ドックパネルと、undock されたパネル 1 枚 1 枚が該当する。いずれも HTML Popover API で top layer
に置く。
...
top layer の順序は `showPopover()` の呼び出し順が SSOT で z-index では越えられないため、前面化の
手段は「hide してから show し直す」しかない。開閉と前面化を同じモジュール（`shared/surface`）に
集約するのは、分散すると「今どれが最前面か」を誰も知らない状態になるため。

この積み直しは見た目のコストを持たない。hide と show の間にジオメトリ読み取りを挟まなければ
スタイル再計算は次のレンダリング更新まで遅延され、`display: none` の状態が具現化しないため、
スクロール位置のリセットも入場アニメーションの再生も起きない。z-index による重ね順管理を
持たないのはこのため。

メニュー / モーダル / トーストはサーフェスに含めない。サーフェスより常に手前でよく、順序の
入れ替えを持たない。
