...
ターミナル全体のスクロールバックは保存しない代わりに、Claude Code セッションの sessionId だけを worktree 単位で永続化する（`~/.config/gozd/projects/<projectKey>/claude-sessions.json`）。`SessionStart` hook は CLI 経由で `session_id` を渡し、`upsert` で保存する。
...
resume 可能なセッションはサイドバーの TaskRow に `resumable` state アイコン（`square-play`）として表示する。worktree 単位の集計バッジは出さない (TaskRow が task 1 件単位で正確に状態を出すため、wt 行に集計を出すと同じ情報が二重表示になる)。Claude 以外のターミナルは保存対象ではない。

#### 永続化エントリの削除経路

`claude-sessions.json` からエントリが消える経路は 4 つ。削除の決定点は **ユーザーの明示的操作 / Claude の明示的終了 / resume 失敗の reactive 検出** に置き、PTY exit そのものは削除契機にしない。アプリ終了時は renderer ごと死んで `unregisterPane` が走らないため、永続化はそのまま残って次回 resume できる。proactive な transcript ファイル存在チェックはしない (Claude 側の transcript 仕様への依存を避けるため)。

| 経路                                                                             | 契機                                                                                             | 動作                                                                                                                                                  |
| -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| renderer の `unregisterPane`（terminal.closePane / resetLayout / worktree 削除） | ユーザーの明示的 pane 削除                                                                       | `rpcClaudeSessionRemoveByPty({ ptyId, worktreePath })` → native が `PTYRegistry` の per-pty sessionId マッピングを解決して `removeBySessionId` を呼ぶ |
| `SessionEnd` hook                                                                | Claude 自身の正規終了（`/exit` 等）                                                              | CLI 経由で `session_id` を渡し `removeBySessionId`                                                                                                    |
| 同 PTY 内の `/clear` / `--resume`                                                | 新 `session-start` 到達時、旧 sessionId と比較                                                   | 旧 ID を `removeBySessionId`                                                                                                                          |
| resume 失敗検出 (`removeByPty` 経路)                                             | spawn 時 `GOZD_RESUME_CLAUDE_SESSION` 期待 sid が SessionStart hook 不達のまま pane が閉じられた | 期待 sid を `removeBySessionId` で削除し、対応する task も `clearDeadSession` で sid をクリア (ghRef 無しなら task 削除)                              |
...
> [!NOTE]
> proto schema から `ClaudeSession.transcript_path` と `HookMessage.transcript_path` を削除した。ベータ版前提で永続データに後方互換を作らないポリシー (CLAUDE.md) に従い `reserved` は入れない。本変更を含むビルドの初回起動時、既存の `claude-sessions.json` が新 schema で parse 失敗するため `ClaudeSessionStore.loadFile` が空ファイルで上書き reinit する。アプリ未起動の状態で claude session を持っていたユーザーは初回起動時に resume 候補が一旦消える (新規起動で再蓄積される)。
