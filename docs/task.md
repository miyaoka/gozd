...
- 同 sessionId が既に attach 済み → no-op (重複 hook / 復元レース)。当該 task が `hidden=true` でも触らない (hidden な ghRef task の蘇生は picker upsert に限定する規律)
...
**hidden な task を候補から外す**のは、terminal close 済みの ghRef task に、別経路で起動した素 claude が偶発的に取り憑くのを構造的に防ぐため。ghRef task の蘇生経路は「PR/issue picker での再選択 → `TaskStore.add` の upsert」に一本化される。
...
| ターミナル close (PTY 終了)              | `detachSession`: sessionId 切り離し。`ghRef` 無しなら task 削除。`ghRef` ありなら `hidden=true`                                                                                                                | PR/issue picker で同じ `ghRef` を再選択 → `add` の upsert で `hidden=false` |
