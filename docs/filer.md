...
`.git` という名前の entry はディレクトリ / ファイルを問わず常にツリーから隠す。判定は native の `FSOps.readDir` で `lastPathComponent == ".git"` をハードコードで除外する。

- 通常 repo の `.git` directory と、worktree / submodule の `.git` gitlink ファイル（`gitdir: ...` を内容に持つ regular file）を同じ判定で消す
- git 本体が `dir.c` で同じ規律を持つ（`is_dot_or_dotdot` と並列に `lastPathComponent == ".git"` を `path_none` 扱い、コメント `"we ignore the name ".git" (even if it is not a directory). That likely will not change."`）。VSCode も `files.exclude` のデフォルト `**/.git: true` で同じ挙動。gozd の filer もこの慣例に従う
