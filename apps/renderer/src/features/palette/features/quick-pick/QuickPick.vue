    ...
    >
      ...
      <ul v-if="filteredItems.length > 0" ref="list" class="max-h-[300px] overflow-y-auto py-1">
        <template v-for="(item, i) in filteredItems" :key="item.label">
          ...
          <li
            v-if="item.separator"
            class="border-t border-border px-3 pt-2 pb-1 text-xs font-semibold tracking-wide text-foreground-subtle"
            :class="{ 'mt-1': i > 0 }"
          >
            {{ item.label }}
          </li>
          ...
          >
            ...
            <span v-if="item.description" class="ml-4 shrink-0 text-xs text-foreground-subtle">
              {{ item.description }}
            </span>
          </li>
        </template>
