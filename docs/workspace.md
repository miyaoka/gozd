...
（開いたのに何も見えない状態を作らない）。窓口はリストに載せない（下記「サイドバー」）。
...
**窓口（[concierge.md](concierge.md)）はプールの外に置く唯一の project**。どのリストにも属さず、
state にも保存しない。サイドバーの最上部に固定で出るので不可視にはならない。どの project の dir か
の判定・ファイル監視・PTY はプールの project と同じに扱う。
