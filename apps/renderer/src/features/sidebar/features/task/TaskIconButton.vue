<doc lang="md">
Task アイコンをクリックして変更するボタン。クリックで popover ピッカーを表示し、選択で即座に emit する。
...
import { TASK_ICONS } from "@gozd/rpc";
...
const id = useId();
const anchorName = `--task-icon-${id}`;
const popoverId = `task-icon-popover-${id}`;
...
<template>
  ...
  >
    <div class="flex flex-wrap gap-0.5" role="group" aria-label="Task icon">
      <button
        v-for="ic in TASK_ICONS"
        :key="ic.emoji"
        type="button"
        :title="ic.title"
        :aria-label="ic.title"
