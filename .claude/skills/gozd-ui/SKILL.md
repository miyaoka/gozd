...
| ----------------------------------------------------- | --------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| filter toggle (強い、active/inactive 2 state)         | primary   | solid static                                             | active=`bg-primary text-primary-foreground` / inactive=`bg-surface-2 text-foreground-muted hover:text-foreground-strong` (assignee:me / reviewer:me)                             |
| view-mode toggle (補助)                               | info      | text-only                                                | split / unified / preview / wordwrap                                                                                                                                             |
| sub-toggle icon                                       | info      | text-only                                                | blame / history switch                                                                                                                                                           |
| mode tab (border indicator)                           | primary   | indicator stripe                                         | preview pane mode tab (`border-b-2 border-primary text-primary`)                                                                                                                 |
| neutral active / selected background (intent なし)    | (neutral) | accent system                                            | `bg-accent-strong text-foreground-strong` を使う要素全般: SettingsModal active tab / palette dialog selected item / SidebarPane view mode toggle / FileTreeItem selected file 等 |
...
| 補助 drag handle (display tweak / max-height 調整 等) | (neutral) | Surface table 参照 (3 state を neutral token で切り替え) | SessionLogTimeline active=`bg-foreground-muted`, rest=`bg-surface-2`, hover=`bg-border-strong`                                                                                   |
