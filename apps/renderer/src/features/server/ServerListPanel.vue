...
preview popover と同流儀)。z-index 競争から構造的に離脱でき、MainLayout の ESC ハンドラが
`:popover-open` で前面判定しているため「前面の popover から順に閉じる」優先順位にも自動で乗る。
開閉の SSOT は store の `isOpen` のままで、popover DOM へは単方向ミラーする。
</doc>
...
// store の isOpen (SSOT) を popover DOM に単方向ミラーする。show/hide は状態不一致の
// 呼び出しで InvalidStateError を投げるため :popover-open で現状態をガードする
// (popover visibility state の変化は同期で、queue されるのは toggle event のみ)。
// flush: "post" で mount 後の DOM に対して初期状態を反映する。
watch(
  [() => serverStore.isOpen, panelRef],
  ([open, el]) => {
    if (!el) return;
