...
<template>
  <Popover
    class="m-0 min-w-44 rounded-lg border border-border bg-background py-1 text-sm text-foreground-strong shadow-lg"
    :style="popoverStyle"
  >
    <button
      v-if="context"
    ...
    >
      ...
      Reset (mixed) to <span class="font-mono text-foreground-muted">{{ shortHash }}</span>
    </button>
  </Popover>
</template>
