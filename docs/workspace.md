...
- **消え切るまでの実体は、gozd のものと持ち主が判る名前で残る**。worktree の一覧には現れず、
  worktree がその名前を取ることもない
- **削除中の worktree の登録は、gozd の他の操作からは触らせない**。触ると削除が中断され、
  登録の無い実体が残る
- **消し切れなかった実体は次の起動で掃除する**。掃除の対象は gozd が置いた worktree の範囲で、
  かつ持ち主のインスタンスが終了しているものに限る
