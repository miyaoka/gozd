...
- base 端は ref 名ではなく commit で持つ。理由は PR 単体の base 端と同じで、rename や force-push を
  またいで同じ端を指せるため
- **stack 1 本ごとの追加往復を撃たない**。stack の帰属は PR 一覧と同じ往復で得られる範囲に収める
  （上の「1 件あたりの追加往復を要する情報は載せない」と同じ制約）
- 帰属が欠けて解決できないときは stack なしとして扱う。**誤った base 端で差分を出さない**。ただし
  黙って落とすと「stack なのに stack の差分が出ない」が診断できないため、観察ログを残す
