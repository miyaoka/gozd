<doc lang="md">
端末 1 つ分の描画実体。PTY との入出力を繋ぎ、端末の中でしか観測できないものを扱う。

## 端末出力は信用しない

画面に出ている文字列は、端末の中で動くプログラムが自由に書けるものであって gozd の管理下に
ない。**リンクの形をしていても、素のクリックでは開かない**。URL もファイルパスも同じ扱いで、
UI が描いたリンクとは起動に要する操作が違う。

## 観測できるものと、その行き先

端末の中でしか観測できないものが 3 つある。端末が名乗ったタイトル、シェルが遷移した作業
ディレクトリ、いま画面に見えている本文。行き先はそれぞれ違う。

- **タイトルと可視本文は外へ渡す**。どう使うか（どの表示に載せるか、何を読み取るか）は
  受け取る側が決める
- **作業ディレクトリは外へ出さず、ここで消費する**。出力行に現れた相対パスをどこ基準で
  解決するかの基準になる（契約は [docs/terminal.md](../../../../../docs/terminal.md)）
