...
native → renderer の push は `WebPage.callJavaScript("window.__gozdReceive(type, payload)", ...)` で行う。配送結果は `pushToRenderer` ヘルパー（`apps/native/Sources/Gozd/WebPagePush.swift`）が stderr にログする。silent drop 禁止規律の根拠は「1 度の取りこぼしで UI 状態が永続的にずれる」こと。

「renderer not ready」は 2 段で起きる: page 未初期化（`page == nil`）と、page はあるが receiver（`window.__gozdReceive`）未登録。後者は HTML ロード完了（JS context が live）と JS bundle 実行完了（`main.ts` の `initRpcDispatcher()` が走り receiver 登録）のラグで生じ、dev では Vite dev server からモジュールグラフを HTTP fetch する分この窓が広い（起動直後だけでなく Vite フルリロードのたびに再発生する）。`callJavaScript` は JS context が live なら即実行するため、receiver 未登録時に素で叩くと `__gozdReceive is not a function` の JS 例外になる。`pushToRenderer` は receiver の有無を JS 側で判定して未登録なら drop する。

ログ方針は失敗 3 種で分ける。page 未初期化は `[GozdApp] push dropped (page not ready)`、JS 例外は `[GozdApp] push failed` としてログする（どちらも「永続的にずれる」性質を持つため観察必須）。receiver 未登録だけは黙って drop する — これは renderer がドキュメント再構築中（起動直後 / Vite フルリロード）の一過性の状態で、その後 renderer は mount で pull hydrate し onMessage 購読も貼り直すため落とした push が構造的に回復し、規律の根拠を満たさないため。native 側に「過去 ready だったか」の履歴フラグを持つと、フルリロードのたびに前ドキュメントの ready 状態が残って偽陽性の drop ログを生む（receiver の有無こそ ready の唯一の真実なので、履歴は持たず毎回 JS 側の bool で現在状態だけを見る）。タイマー駆動の push（PortScanner の `serverPortsChange`）が bootstrap 窓に当たるが、renderer mount 時の `/server/list` pull hydrate（`useServerStore`）が回収する。`PortScanner` は差分時のみ push する（churn 抑制）ため周期再送は回収を担保しない点に注意（回収経路は pull hydrate のみ）。
