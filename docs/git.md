...
- 作業ツリー側のファイル変更は gitignore 対象 (`*.tsbuildinfo` / `dist` / `node_modules` 等のビルド成果物) でも `gitStatusChange` に分類される (git dir 外の変更は untracked / 差分の可能性があるため一律立てる)。だが `git status` 出力は ignore ファイルを除外するので `StatusFull` は前回と同一になる。typecheck / ビルド中はこの「内容不変の push」が連射され、renderer 側 `setWorktreeGitStatuses` の参照差し替えを通じて changes / filer ビューが再描画され続ける。内容一致 dedup でこれを止める
